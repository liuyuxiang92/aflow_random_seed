...
namespace apl {
  ...
  void ThermalPropertiesCalculator::calculateThermalProperties(double Tstart,
    ...
    if (Tstart > Tend) {
      string function_name = XPID + _APL_THERMO_ERR_PREFIX_ + "calculateThermalProperties():";
...
namespace apl {
  ...
  double ThermalPropertiesCalculator::getStepDOS(const vector<double>& freq) {
    ...
    if (freq.size() > 2) {
      ...
      string function_name = XPID + _APL_THERMO_ERR_PREFIX_ + "getStepDOS():";
...
namespace apl {
  ...
  void ThermalPropertiesCalculator::writePropertiesToFile(string filename, filetype ft) {
    ...
    if (!aurostd::FileExist(filename)) {
      string function_name = XPID + "ThermalPropertiesCalculator::writePropertiesToFile():";
