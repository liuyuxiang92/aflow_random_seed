...
void TCONDCalculator::buildQpoints(const xvector<int>& qgrid) {
  ...
  if (!pcell.pgroupk_xtal_calculated) {
...
void TCONDCalculator::writeQpoints() {
  stringstream output;
  string filename = DEFAULT_AAPL_FILE_PREFIX + DEFAULT_AAPL_QPOINTS_FILE;
  ...
  for (int q = 0; q < nQPs; q++) {
    ...
    for (int i = 1; i < 4; i++) {
      ...
      output << std::setw(20) << std::setprecision(10) << std::scientific << qpoints[q].cpos[i];
    }
    output << std::endl;
  }
...
void TCONDCalculator::writeIrredQpoints() {
  stringstream output;
  string filename = DEFAULT_AAPL_FILE_PREFIX + DEFAULT_AAPL_IRRQPTS_FILE;
  ...
  for (int iq = 0; iq < nIQPs; iq++) {
    ...
    output << std::setw(10) << q;
    output << std::setiosflags(std::ios::fixed | std::ios::right);
    output << std::setw(15) << irred_qpoints[iq].size();
    for (int i = 1; i < 4; i++) {
      output << std::setiosflags(std::ios::fixed | std::ios::showpoint | std::ios::right);
      output << std::setw(20) << std::setprecision(10) << std::scientific << qpoints[q].cpos[i];
    }
...
void TCONDCalculator::writeTempIndepRatesFile(string tag, string filename,
  ...
  for (int iq = 0; iq < nIQPs; iq++) {
    output << std::setiosflags(std::ios::fixed | std::ios::right);
    output << std::setw(10) << irred_qpoints[iq][0];
    for (int br = 0; br < nBranches; br++) {
      output << std::setw(20) << std::setprecision(10) << std::scientific << rates[iq][br];
    }
    output << std::endl;
  }
...
string TCONDCalculator::tempDepRatesString(string tag, double temp,
  ...
  ratestream << "= " << std::setprecision(2) << std::fixed << temp << " K]START" << std::endl;
  ratestream << std::setw(10) << "# Q-point";
  ratestream << std::setw(20) << " ";
  ratestream << "Scattering rates (1/ps)" << std::endl;
  for (int iq = 0; iq < nIQPs; iq++) {
    ratestream << std::setiosflags(std::ios::fixed | std::ios::right);
    ratestream << std::setw(10) << irred_qpoints[iq][0];
    for (int br = 0; br < nBranches; br++) {
      ratestream << std::setw(20) << std::setprecision(10) << std::scientific << rates[iq][br];
    }
    ratestream << std::endl;
  }
  ratestream << "[AAPL_SCATTERING_RATE_" << aurostd::toupper(tag);
