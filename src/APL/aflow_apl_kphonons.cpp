...
namespace KBIN {
  ...
  void RunPhonons_APL_20181216(_xinput& xinput,
    ...
    if (LDEBUG) std::cerr << __func__ << " DEBUG [0]" << std::endl;
    // Test
    ...
    if (LDEBUG) std::cerr << __func__ << " DEBUG [1a]" << std::endl;
    ...
    if (LDEBUG) std::cerr << __func__ << " DEBUG [1b]" << std::endl;
    ...
    if (LDEBUG) std::cerr << __func__ << " DEBUG [1c]" << std::endl;
    ...
    if (LDEBUG) std::cerr << __func__ << " DEBUG [2]" << std::endl;
      ...
      if(xflags.AFLOW_MODE_VASP && !XHOST.GENERATE_AFLOWIN_ONLY){  //ME20190313 - Do not check the VASP binary for generate_aflowin_only
        try {
          ...
          if (aurostd::isequal(vaspVersion,0.0)==false){  //CO20210315
            ...
            message << "Failed.";
            pflow::logger(_AFLOW_FILE_NAME_, modulename, message, aflags, FileMESSAGE, oss, _LOGGER_WARNING_);
            throw aurostd::xerror(_AFLOW_FILE_NAME_, __func__, "Unexpected binary format.", _FILE_WRONG_FORMAT_);
          }
    ...
    if (LDEBUG) std::cerr << __func__ << " DEBUG [3b]" << std::endl;
    ...
    if (LDEBUG) std::cerr << __func__ << " DEBUG [5a]" << std::endl;
    ...
    if (!USER_FREQFORMAT.empty()) {
      ...
      if (((frequencyFormat & ~apl::OMEGA) & ~apl::ALLOW_NEGATIVE) == apl::NONE)
        throw aurostd::xerror(_AFLOW_FILE_NAME_, __func__, "Ambiguous frequency format.", _INPUT_AMBIGUOUS_);
    } else {
    ...
    if (LDEBUG) std::cerr << __func__ << " DEBUG [5b]" << std::endl;
    ...
    if (LDEBUG) std::cerr << __func__ << " DEBUG [5c]" << std::endl;
    ...
    if (LDEBUG) std::cerr << __func__ << " DEBUG [6]" << std::endl;
...
namespace apl {
  ...
  void validateParametersSupercellAPL(aurostd::xoption& aplopts) {
    ...
    if (aplopts.flag("SUPERCELL_ENTRY")) {
      ...
      if (tokens.size() != 3 && tokens.size() != 9) {
        ...
        throw aurostd::xerror(_AFLOW_FILE_NAME_, __func__, message, _INPUT_NUMBER_);
      }
    ...
    if (aplopts.flag("DOS_PROJECT")) {
      ...
      if (!dos_proj_frac.empty() || !dos_proj_cart.empty()) {
        if (!dos_proj_cart.empty() && !dos_proj_frac.empty()) {
          ...
          throw aurostd::xerror(_AFLOW_FILE_NAME_, __func__, message, _INPUT_AMBIGUOUS_);
        } else {
          string projscheme;
  ...
  void validateParametersQHA(xoption& qhaopts, const _aflags& aflags, ofstream& FileMESSAGE, ostream& oss)
  {
    ...
    pflow::logger(_AFLOW_FILE_NAME_, __func__, "Validating QHA parameters.", aflags,
    ...
    if (dtokens.size() != 3) {
      ...
      throw aurostd::xerror(_AFLOW_FILE_NAME_, __func__, msg, _INPUT_NUMBER_);
    }
