  ...
  bool runRelaxationsAPL_VASP(int start_relax, const string& AflowIn, _xvasp& xvasp,
    ...
    if (Krun) {
      ...
      for (i = start_relax; Krun && i <= _NUM_RELAX_; i++) {
        aus << 11111*i << " RELAXATION APL - " << xvasp.Directory << " - K=[" << xvasp.str.kpoints_k1 << " " << xvasp.str.kpoints_k2 << " " << xvasp.str.kpoints_k3 << "]" << " - " << kflags.KBIN_BIN << " - " << Message(__AFLOW_FILE__) << endl;
        ...
        if (i < _NUM_RELAX_) {
          ...
          XVASP_INCAR_SPIN_FIX_RELAX(xvasp, aflags, vflags, i, true, fileMessage); //CO20210315 - always write_incar
          ...
          XVASP_INCAR_SPIN_FIX_RELAX(xvasp, aflags, vflags, i, false, fileMessage);  //ME20200115 - or else SPIN_REMOVE_RELAX_2 does not work  //CO20210315 - write_incar only if (i<num_relax), no static afterward
        }
      }
      if (Krun && (i == _NUM_RELAX_)) {
        aus << 11111*i << " RELAXATION APL END - " << xvasp.Directory << " - K=[" << xvasp.str.kpoints_k1 << " " << xvasp.str.kpoints_k2 << " " << xvasp.str.kpoints_k3 << "]" << " - " << kflags.KBIN_BIN << " - " << Message(__AFLOW_FILE__) << endl;
      }
    }
...
namespace apl {
  ...
  void validateParametersQHA(xoption& qhaopts, const _aflags& aflags, ofstream& FileMESSAGE, ostream& oss)
  {
    ...
    pflow::logger(__AFLOW_FILE__, __AFLOW_FUNC__, "Validating QHA parameters.", aflags,
