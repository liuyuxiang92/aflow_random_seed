...
namespace apl {
  ...
  xvector<int> Supercell::determineSupercellDimensions(const aurostd::xoption& opts) {
    ...
    if (method == "SUPERCELL") {
      ...
      aurostd::xerror(_AFLOW_FILE_NAME_, __func__, message, _VALUE_ILLEGAL_);
    }
    return dims;
  }
  ...
  bool Supercell::projectToPrimitive() {
    ...
    if (!mapped) {
      ...
      if (LDEBUG) std::cerr << __func__ << " Not mapped succesfully. Try shifting atoms." << std::endl;
    ...
    if (mapped) {
      ...
      for (uint iatpc = 0; ((iatpc < niatoms_pc) && mapped); iatpc++) {
        ...
        if (iatoc == niatoms_oc) {
          if (LDEBUG) std::cerr << __func__ << " Did not map iatoms of primitive cell (failed for " << iatpc << ")." << std::endl;
          mapped = false;
        }
      }
    }
  ...
  bool Supercell::getMaps(const xstructure& pcell, const xstructure& ocell, const xstructure& scell,
    ...
    if ((natoms_pc > natoms_oc) || (natoms_oc > natoms_sc)) {
      if (LDEBUG) {
        std::cerr << __func__ << " Cannot map a smaller cell to a larger cell." << std::endl;
      }
      return false;
    }
    ...
    for (uint i = 0; i < natoms_sc; i++) {
      ...
      if (j == natoms_pc) {
        if (LDEBUG) {
          std::cerr << __func__ << " sc2pcMap failed for atom " << i << "." << std::endl;
        }
        return false;
      }
    }
    ...
    for (uint i = 0; i < natoms_pc; i++) {
      ...
      for (j = 0; j < natoms_oc; j++) {
        if (SYM::FPOSMatch(fpos, ocell.atoms[j].fpos, ocell.lattice, ocell.f2c, _skew, _sym_eps)) {
          ...
          if (k == natoms_sc) {
            if (LDEBUG) {
              std::cerr << __func__ << " pc2scMap failed for atom " << i << "."
      ...
      if (j == natoms_oc) {
        if (LDEBUG) {
          std::cerr << __func__ << " pc2scMap failed for atom " << i << "." << std::endl;
        }
        return false;
      }
    }
