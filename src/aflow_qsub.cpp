...
namespace KBIN {
  bool QSUB_Extract(_xqsub& xqsub,string AflowIn,ifstream &FileAFLOWIN,ofstream &FileMESSAGE,_aflags &aflags,_kflags &kflags) {        // AFLOW_FUNCTION_IMPLEMENTATION
    ...
    if(kflags.KBIN_QSUB) {
      aus      << "00000  [AFLOW_MODE_QSUB] found in " << _AFLOWIN_ << " "  << Message(aflags,"user,host,time",_AFLOW_FILE_NAME_) << endl;
	...
	aus << "00000  MESSAGE QSUB: COMMAND string is missing, taking COMMAND=\"" << kflags.KBIN_QSUB_COMMAND << "\"  " << Message(aflags,"user,host,time",_AFLOW_FILE_NAME_) << endl;
	aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);    
      } else {
	kflags.KBIN_QSUB_COMMAND=aurostd::RemoveCharacter(aurostd::substring2string(AflowIn,"[AFLOW_QSUB_MODE]COMMAND=",TRUE),'"');
	aus << "00000  MESSAGE QSUB: found COMMAND=\"" << kflags.KBIN_QSUB_COMMAND << "\"  " << Message(aflags,"user,host,time",_AFLOW_FILE_NAME_) << endl;
	aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);    
      }
	...
	aus << "00000  MESSAGE QSUB: PARAMS string is missing, taking PARAMS=\"" << kflags.KBIN_QSUB_PARAMS << "\"  " << Message(aflags,"user,host,time",_AFLOW_FILE_NAME_) << endl;
	aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);    
      } else {
	kflags.KBIN_QSUB_PARAMS=aurostd::RemoveCharacter(aurostd::substring2string(AflowIn,"[AFLOW_QSUB_MODE]PARAMS=",TRUE),'"');
	aus << "00000  MESSAGE QSUB: found PARAMS=\"" << kflags.KBIN_QSUB_PARAMS << "\"  " << Message(aflags,"user,host,time",_AFLOW_FILE_NAME_) << endl;
	aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);    
      }
    
      Krun=(Krun && kflags.KBIN_QSUB_MODE_EXPLICIT);
      if(!Krun) {
	aus << "EEEEE  [AFLOW_QSUB_MODE_IMPLICIT] is the only supported mode "  << Message(aflags,"user,host,time",_AFLOW_FILE_NAME_) << endl;
	...
	if(kflags.KBIN_QSUB_FILE && !kflags.KBIN_QSUB_MODE_EXPLICIT_START_STOP) {
	  aus << "00000  MESSAGE QSUB   generation EXPLICIT file from " << _AFLOWIN_ << " " << Message(aflags,"user,host,time",_AFLOW_FILE_NAME_) << endl;
	  aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);    
	  ...
	  aus << "00000  MESSAGE QSUB   generation EXPLICIT file from " << _AFLOWIN_ << " with START/STOP  " << Message(aflags,"user,host,time",_AFLOW_FILE_NAME_) << endl;
	  aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
	  ...
	  aus << "EEEEE  [AFLOW_QSUB_MODE_EXPLICIT] do not confuse aflow !!"  << Message(aflags,"user,host,time",_AFLOW_FILE_NAME_) << endl;
	  aus << "EEEEE  [AFLOW_QSUB_MODE_EXPLICIT] Possible modes "  << Message(aflags,"user,host,time",_AFLOW_FILE_NAME_) << endl;
	  aus << "----------------------------------------------------------------------------------------------------" << endl;
	  ...
	  aus << "[AFLOW]" << endl;
	  aus << "----------------------------------------------------------------------------------------------------" << endl;
	  aus << "EEEEE  [AFLOW_QSUB_MODE_EXPLICIT] Note "  << Message(aflags,"user,host,time",_AFLOW_FILE_NAME_) << endl;
	  aus << "EEEEE  [AFLOW_QSUB_MODE_EXPLICIT]START must be present and no [AFLOW_QSUB_FILE]"  << Message(aflags,"user,host,time",_AFLOW_FILE_NAME_) << endl;
	  aus << "EEEEE  [AFLOW_QSUB_MODE_EXPLICIT]STOP  must be present and no [AFLOW_QSUB_FILE]"  << Message(aflags,"user,host,time",_AFLOW_FILE_NAME_) << endl;
	  aus << "EEEEE  or [AFLOW_QSUB_FILE] present and NO START/STOP"  << Message(aflags,"user,host,time",_AFLOW_FILE_NAME_) << endl;
	  aurostd::PrintErrorStream(FileMESSAGE,aus,XHOST.QUIET);    
	  Krun=FALSE;
	  return Krun;
	}      
...
namespace KBIN {
  bool QSUB_RunFinished(_aflags &aflags,ofstream &FileMESSAGE,bool verbose) {
    ...
    if(!aurostd::FileExist(aflags.Directory+"/aflow.qsub.done")) {
      if(verbose) aus << "00000  MESSAGE RUN NOT FINISHED (aflow.qsub.done does not exist) : " << Message(aflags,"user,host,time",_AFLOW_FILE_NAME_) << endl;
      if(verbose) aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
      return FALSE;
    }
    // OTUCAR ESISTS BUT EMPTY
    if(aurostd::FileEmpty(aflags.Directory+"/aflow.qsub.done")) {
      if(verbose) aus << "00000  MESSAGE RUN NOT FINISHED (aflow.qsub.done is empty) : " << Message(aflags,"user,host,time",_AFLOW_FILE_NAME_) << endl;
      if(verbose) aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
      return FALSE;
    }
    ...
    if(aurostd::substring_present_file(aflags.Directory+"/aflow.qsub.done","DONE",TRUE) ||
      ...
      if(verbose) aus << "00000  MESSAGE RUN FINISHED (aflow.qsub.done is complete) : " << Message(aflags,"user,host,time",_AFLOW_FILE_NAME_) << endl;
      if(verbose) aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
      aurostd::execute(aus_exec);    
      return TRUE;
    }
    aurostd::execute(aus_exec);
    if(verbose) aus << "00000  MESSAGE RUN NOT FINISHED (aflow.qsub.done is incomplete) : " << Message(aflags,"user,host,time",_AFLOW_FILE_NAME_) << endl;
    if(verbose) aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
...
namespace KBIN {
  bool QSUB_Extract_Mode1(_xqsub& xqsub,ofstream &FileMESSAGE,_aflags &aflags,_kflags &kflags) {        // AFLOW_FUNCTION_IMPLEMENTATION  
    ...
    aus      << "00000  [AFLOW_MODE_QSUB] QSUB Mode1 "  << Message(aflags,"user,host,time",_AFLOW_FILE_NAME_) << endl;
    aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
    Krun=kflags.KBIN_QSUB;
...
namespace KBIN {
  bool QSUB_Extract_Mode2(_xqsub& xqsub,ofstream &FileMESSAGE,_aflags &aflags,_kflags &kflags) {        // AFLOW_FUNCTION_IMPLEMENTATION  
    ...
    //
    kflags.KBIN_QSUB=TRUE;                         // search for QSUB string
    aus      << "00000  [AFLOW_MODE_QSUB] QSUB Mode2 "  << Message(aflags,"user,host,time",_AFLOW_FILE_NAME_) << endl;
    aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
    Krun=kflags.KBIN_QSUB;
...
namespace KBIN {
  bool QSUB_Extract_Mode3(_xqsub& xqsub,ofstream &FileMESSAGE,_aflags &aflags,_kflags &kflags) {        // AFLOW_FUNCTION_IMPLEMENTATION  
    ...
    //
    kflags.KBIN_QSUB=TRUE;                         // search for QSUB string
    aus      << "00000  [AFLOW_MODE_QSUB] QSUB Mode3 "  << Message(aflags,"user,host,time",_AFLOW_FILE_NAME_) << endl;
    aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
    Krun=kflags.KBIN_QSUB;
