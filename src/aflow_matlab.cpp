...
bool KBIN_MATLAB_Extract(string AflowIn,ifstream &FileAFLOWIN,ofstream &FileMESSAGE,_aflags &aflags,_kflags &kflags) {
  ...
  if(!Krun) {
    aus << "EEEEE  [AFLOW_MATLAB_MODE_EXPLICIT] and [AFLOW_MATLAB_MODE_EXTERNAL] are the only supported modes "  << Message(aflags,_AFLOW_MESSAGE_DEFAULTS_,_AFLOW_FILE_NAME_) << endl;
  ...
  if(Krun && kflags.AFLOW_MATLAB_MODE_EXPLICIT) {  // [MATLAB_MODE_EXPLICIT] construction
    if(kflags.AFLOW_MATLAB_FILE && !kflags.AFLOW_MATLAB_MODE_EXPLICIT_START_STOP) {
      aus << XPID << "00000  MESSAGE MATLAB  generation EXPLICIT file from " << _AFLOWIN_ << " " << Message(aflags,_AFLOW_MESSAGE_DEFAULTS_,_AFLOW_FILE_NAME_) << endl;
      ...
      aus << XPID << "00000  MESSAGE MATLAB  generation EXPLICIT file from " << _AFLOWIN_ << " with START/STOP  " << Message(aflags,_AFLOW_MESSAGE_DEFAULTS_,_AFLOW_FILE_NAME_) << endl;
      aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);    
      ...
      aus << "EEEEE  [MATLAB_MODE_EXPLICIT] do not confuse aflow !!"  << Message(aflags,_AFLOW_MESSAGE_DEFAULTS_,_AFLOW_FILE_NAME_) << endl;
      aus << "EEEEE  [MATLAB_MODE_EXPLICIT] Possible modes "  << Message(aflags,_AFLOW_MESSAGE_DEFAULTS_,_AFLOW_FILE_NAME_) << endl;
      aus << "----------------------------------------------------------------------------------------------------" << endl;
      ...
      aus << "[AFLOW]" << endl;
      aus << "----------------------------------------------------------------------------------------------------" << endl;
      aus << "EEEEE  [MATLAB_MODE_EXPLICIT] Note "  << Message(aflags,_AFLOW_MESSAGE_DEFAULTS_,_AFLOW_FILE_NAME_) << endl;
      aus << "EEEEE  [MATLAB_MODE_EXPLICIT]START must be present and no [AFLOW_MATLAB_FILE]"  << Message(aflags,_AFLOW_MESSAGE_DEFAULTS_,_AFLOW_FILE_NAME_) << endl;
      aus << "EEEEE  [MATLAB_MODE_EXPLICIT]STOP  must be present and no [AFLOW_MATLAB_FILE]"  << Message(aflags,_AFLOW_MESSAGE_DEFAULTS_,_AFLOW_FILE_NAME_) << endl;
      aus << "EEEEE  or [AFLOW_MATLAB_FILE] present and NO START/STOP"  << Message(aflags,_AFLOW_MESSAGE_DEFAULTS_,_AFLOW_FILE_NAME_) << endl;
      aurostd::PrintErrorStream(FileMESSAGE,aus,XHOST.QUIET);    
      Krun=FALSE;
      return Krun;
    }      
  ...
  if(Krun && kflags.AFLOW_MATLAB_MODE_EXTERNAL) {  // [AFLOW_MATLAB_MODE_EXTERNAL] construction
    string file;
    aus << XPID << "00000  MESSAGE aflow.m   generation EXTERNAL file from " << _AFLOWIN_ << " " << Message(aflags,_AFLOW_MESSAGE_DEFAULTS_,_AFLOW_FILE_NAME_) << endl;
    aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);  
    if(kflags.AFLOW_MATLAB_FILE_COMMAND && kflags.AFLOW_MATLAB_FILE_FILE) {
      aus << "EEEEE   [AFLOW_MATLAB_MODE]FILE=  and  [AFLOW_MATLAB_MODE]COMMAND=  can not be used together "  << Message(aflags,_AFLOW_MESSAGE_DEFAULTS_,_AFLOW_FILE_NAME_) << endl;
      aurostd::PrintErrorStream(FileMESSAGE,aus,XHOST.QUIET);    
      Krun=FALSE;
      return Krun;
    }
    if(!kflags.AFLOW_MATLAB_FILE_COMMAND && (kflags.AFLOW_MATLAB_FILE_FILE || !kflags.AFLOW_MATLAB_FILE_FILE)) {
      if(kflags.AFLOW_MATLAB_FILE_FILE) {
        ...
        aus << XPID << "00000  MESSAGE aflow.m   generation from file=" << file << endl;
        aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);  
      } else {
        file=DEFAULT_VASP_EXTERNAL_INCAR;
        aus << XPID << "00000  MESSAGE aflow.m   generation from DEFAULT file=" << DEFAULT_VASP_EXTERNAL_INCAR << Message(aflags,_AFLOW_MESSAGE_DEFAULTS_,_AFLOW_FILE_NAME_) << endl;
        aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);  
      }
      if(!aurostd::FileExist(file)) {
        aus << "EEEEE  ERROR aflow.m file=" << file << " does not exist! " << Message(aflags,_AFLOW_MESSAGE_DEFAULTS_,_AFLOW_FILE_NAME_) << endl;
        aurostd::PrintErrorStream(FileMESSAGE,aus,XHOST.QUIET);    
        Krun=FALSE;
        return Krun;
      }
      if(aurostd::FileEmpty(file)) {
        aus << "EEEEE  ERROR aflow.m file=" << file << " is empty! " << Message(aflags,_AFLOW_MESSAGE_DEFAULTS_,_AFLOW_FILE_NAME_) << endl;
        aurostd::PrintErrorStream(FileMESSAGE,aus,XHOST.QUIET);    
        Krun=FALSE;
        return Krun;
      }
    ...
    if(kflags.AFLOW_MATLAB_FILE_COMMAND && !kflags.AFLOW_MATLAB_FILE_FILE) {
      ...
      aus << XPID << "00000  MESSAGE aflow.m   generation from command= '" << file << "' " << endl;
      aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);  
...
bool KBIN_MATLAB_Directory(ofstream &FileMESSAGE,_aflags &aflags,_kflags &kflags) { // AFLOW_FUNCTION_IMPLEMENTATION
  ...
  aurostd::StringstreamClean(aus);
  aus << XPID << "00000  MESSAGE KBIN_MATLAB_Directory Running KBIN_BIN=\"" << kflags.KBIN_BIN << "\" " << Message(aflags,_AFLOW_MESSAGE_DEFAULTS_,_AFLOW_FILE_NAME_) << endl;
  aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
  // ***************************************************************************
  ...
  if(Krun) {
    ...
    if(!DirectoryStream) {
      aus << "XXXXX  MAKING DIRECTORY = " << aflags.Directory << "  " << Message(aflags,_AFLOW_MESSAGE_DEFAULTS_,_AFLOW_FILE_NAME_) << endl;
    ...
    if(Krun==TRUE) {   // survived all troubles
      ...
      // ***************************************************************************
      // RUN
      aus << "AAAAA  MATLAB RUN - " <<  aflags.Directory << " - " << kflags.KBIN_BIN << " - " << Message(_AFLOW_MESSAGE_DEFAULTS_,_AFLOW_FILE_NAME_) << endl;
      aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
