...
//DX20190214 [OBSOLETE]  // DX and CO - START
...
//DX20190214 [OBSOLETE]     // DX and CO - END
...
void atoms_initialize(void) {
  for(int i=0;i<NUM_ELEMENTS;i++) {       // clear
    ...
    vatom_radius_covalent.at(i)=NNN;    // Codero (Angstroms) // DX and CO -20170904
  ...
  i++; vatom_symbol[i]="C";  vatom_name[i]="Carbon";       vatom_mass[i]*=12.011;    vatom_volume[i]= 5.59490; vatom_valence_std[i]=4; vatom_valence_iupac[i]=4;  vatom_miedema_phi_star[i]=6.20;  vatom_miedema_nws[i]=1.90;  vatom_miedema_Vm[i]=1.8;  vatom_miedema_gamma_s[i]= NNN;   vatom_miedema_BVm[i]=NNN;   vatom_radius[i]=0.077;  vatom_radius_covalent[i]=0.76;   vatom_electronegativity[i]=2.55;  vatom_crystal[i]="dia";  vatom_pettifor_scale[i]=2.50; vatom_xray_scatt[i]=6.019; vatom_pearson_coefficient[i]=0.00007387218;   // C  // DX and CO -20170904 vatom_radius_covalent uses sp3 hybridization (most common)
...
#define oss cout
  ...
  for(uint i=1;i<vinput.size();i++) { // not 1st line
    //aurostd::string2tokens(vinput[i],tokens,"//");  // stefano   // CO20180409 - not the best way, as it will screw up for lines that START with comment delimiter
      ...
      for(int i=-1;i<=1&&FOUND_POSITION==FALSE;i++)
        for(int j=-1;j<=1&&FOUND_POSITION==FALSE;j++)
          for(int k=-1;k<=1&&FOUND_POSITION==FALSE;k++) {
            ...
            if((*this).sym_eps!=AUROSTD_NAN && (*this).sym_eps<AUROSTD_NAN && (*this).sym_eps>1e-10){ // DX20170107 - Added (*this).sym_eps>1e-10 // DX20180215 - added (*this).sym_eps<AUROSTD_NAN (needed) 
              ...
              if(aurostd::modulus(atoms.at(iat).cpos-(((double)i)*a1+((double)j)*a2+((double)k)*a3+atom.cpos))<0.1) FOUND_POSITION=TRUE; // DX20170107
            }
            // DX and CO - END
          }
...
//DX20190905 [OBSOLETE] //USE THE ONE THAT ONLY COPIES ATOMS, COREY
