...
uint xstructure::GetPrimitiveCell(void) {
  ...
  for (int i = 0; i < atom_count; i++) {
    ...
    //DX20191011 [OBSOLETE] tmp_atom.spin = (*this).atoms[i].spin; // DX 9/21/17 - magnetic sym
    //DX20191011 [OBSOLETE] tmp_atom.spin_is_given = (*this).atoms[i].spin_is_given; // DX 9/21/17 - magnetic sym
  ...
  if(foundbasis == false) {
    for (uint i = 0; i < atomic_basis_.size(); i++) {
      ...
      //DX20191011 [OBSOLETE] tmp.spin = atomic_basis_[i].spin; // DX 9/21/17 - magnetic sym
      //DX20191011 [OBSOLETE] tmp.spin_is_given = atomic_basis_[i].spin_is_given; // DX 9/21/17 - magnetic sym
    ...
    for (uint d = 1; d < diff_vectors.size(); d++) {  //use d=0 if we do double for loop
      ...
      for (uint i = 0; i < atomic_basis_.size(); i++) {
        ...
        //DX20191011 [OBSOLETE] atmp.spin = atomic_basis_[i].spin; // DX 9/21/17 - magnetic sym
        //DX20191011 [OBSOLETE] atmp.spin_is_given = atomic_basis_[i].spin_is_given; // DX 9/21/17 - magnetic sym
    ...
    if(num_of_candidates == 3) {
      ...
      //[CO190520]newbasis = foldAtomsInCell(expanded_basis, c2f, f2c, skew, same_atom_tol); // CO20180409
  ...
  if(newbasis.size() > 0) {  //IF A NEW BASIS EXISTS (CELL WAS NOT PRIMITIVE) THEN OVERWRITE a.atoms
    ...
    for (uint i = 0; i < newbasis.size(); i++) {
      ...
      //DX20191011 [OBSOLETE] tmp_atom.spin = newbasis[i].spin; // DX 9/21/17 - magnetic sym
  ...
  deque<_atom> add_basis(vector<xvector<double> >& expanded_lattice_points, xmatrix<double>& L, xstructure& xstr) {
    ...
    for (uint i = 0; i < expanded_lattice_points.size(); i++) {
      ...
      for (uint j = 0; j < xstr.atoms.size(); j++) {  // (OBSOLETE) go from 1 because
        ...
        //DX20191011 [OBSOLETE] tmp.spin = xstr.atoms[j].spin; // DX 9/21/17 - magnetic sym
        //DX20191011 [OBSOLETE] tmp.spin_is_given = xstr.atoms[j].spin_is_given; // DX 9/21/17 - magnetic sym
  ...
  xstructure ConventionalCell(xstructure& xstr, int& IT, int& cell_choice, bool& last_orientation, string& crystalsystem_prev,
    ...
    while (reform == true) {
      ...
      for (uint i = start_index; i < candidate_lattice_vectors.size(); i++) {
        ...
        //[CO190520]deque<_atom> conventional_basis_atoms = foldAtomsInCell(expanded_basis, c2f, f2c, skew, sym_tol); //DX20190215 - _SYM_TOL_ to sym_tol
...
string xstructure::aflow2sg(void) {
  ...
  // DX20170509 - [OBSOLETE] double orig_tolerance = use_tol;
  bool no_scan = false;
  // DX20170509 - [OBSOLETE] uint sgroup = (*this).SpaceGroup_ITC(use_tol, orig_tolerance, -1, change_sym_count, no_scan);
...
string xstructure::aflow2sg(double& use_tol, const int& manual_it) {
  ...
  // DX20170509 - [OBSOLETE] int change_sym_count = 1;
  bool no_scan = false;
  // DX20170509 - [OBSOLETE] uint sgroup = (*this).SpaceGroup_ITC(use_tol, orig_tolerance, manual_it, change_sym_count, no_scan);
...
uint xstructure::SpaceGroup_ITC(void) {
  ...
  // DX20170509 - [OBSOLETE] int change_sym_count = 1;
  bool no_scan = false;
  // DX20170509 - [OBSOLETE] return (*this).SpaceGroup_ITC(use_tol, orig_tolerance, -1, change_sym_count, no_scan);
...
//  // DX20170509 - [OBSOLETE] int change_sym_count = 1;
//  bool no_scan = false;
//  // DX20170509 - [OBSOLETE] return (*this).SpaceGroup_ITC(use_tol, orig_tolerance, manual_it, change_sym_count, no_scan);
...
// ********************************************************************************************************************************
// DX20170509 - [OBSOLETE] uint xstructure::SpaceGroup_ITC(double& use_tol, double& orig_tolerance, const int& manual_it, int& change_sym_count, bool& no_scan)
