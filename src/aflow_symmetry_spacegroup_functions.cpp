...
//DX 20190215 - added new class - START
...
// ******************************************************************************
namespace SYM {
  void ReducedRowEchelonForm(xmatrix<double>& M, double& tol) { //DX 20190215 -added tol
    //DX 20190215 [OBSOLETE] double tol = _SYM_TOL_;
    uint lead = 1;
    uint rowCount = M.urows;
    uint columnCount = M.ucols;
...
// ******************************************************************************
namespace SYM {
  bool find_solution_UD(xmatrix<double> M, xvector<double>& SOL, double& tol) { //DX 20190215 -added tol
...
namespace SYM {
  bool checkLinearSystem(vector<xvector<double> >& LHS, vector<double>& RHS, xmatrix<double>& lattice, double& tol) { //DX 20190215 -added tol
    //double tol = 5e-3;
    //DX 20190215 [OBSOLETE] double tol = _SYM_TOL_;
    bool allzero = true;
...
// ******************************************************************************
namespace SYM {
  bool solve_overdetermined_system(vector<xvector<double> >& LHS, vector<double>& RHS, xvector<double>& SOL, xmatrix<double>& lattice, double& min_dist, double& tol) { //DX 20190215 - added tol
      ...
      if(line > 1) {
        ...
        if(spaceg[i] == 0x0a) {
          ...
          if(letter == Wyckoff_letter){
            for(uint m=0;m<Wyckoff_multiplicity;m++){ //DX 20190128 - int to uint
              temp >> pos;
              positions.push_back(pos);
            }
            break;
...
namespace SYM {
  void getGeneralWyckoffMultiplicityAndPosition(uint space_group_number, string& space_group_setting, int& general_wyckoff_multiplicity, vector<string>& general_wyckoff_position){
    bool LDEBUG = (FALSE || XHOST.DEBUG);
    SymmetryInformationITC ITC_sym_info; //DX 20190215
    ITC_sym_info.initsgs(space_group_setting); //DX 20190215
...
// DX 20200313 [reformatted and moved to AUROSTD]// ******************************************************************************
// DX 20200313 [reformatted and moved to AUROSTD]// dbl2frac Double to Fraction (Overloaded)
// DX 20200313 [reformatted and moved to AUROSTD]// ******************************************************************************
// DX 20200313 [reformatted and moved to AUROSTD]//NEED TO REWRITE/check FRAC2DBL
// DX 20200313 [reformatted and moved to AUROSTD]namespace SYM {
// DX 20200313 [reformatted and moved to AUROSTD]  double frac2dbl(string str) {  //string in the form "a/b" with minus if negative
// DX 20200313 [reformatted and moved to AUROSTD]    //  if(str[0]==' '){
// DX 20200313 [reformatted and moved to AUROSTD]    //str.erase(str.begin(),str.begin()+1);
// DX 20200313 [reformatted and moved to AUROSTD]    //}
// DX 20200313 [reformatted and moved to AUROSTD]
// DX 20200313 [reformatted and moved to AUROSTD]    double out = 0;
// DX 20200313 [reformatted and moved to AUROSTD]    bool neg = false;
// DX 20200313 [reformatted and moved to AUROSTD]    stringstream ss_num, ss_den;
// DX 20200313 [reformatted and moved to AUROSTD]    cleanupstring(str);
// DX 20200313 [reformatted and moved to AUROSTD]    if(str[0] == '-') {
// DX 20200313 [reformatted and moved to AUROSTD]      neg = true;
// DX 20200313 [reformatted and moved to AUROSTD]      str.erase(str.begin(), str.begin() + 1);
// DX 20200313 [reformatted and moved to AUROSTD]    }
// DX 20200313 [reformatted and moved to AUROSTD]    if(str[0] == '+') {
// DX 20200313 [reformatted and moved to AUROSTD]      str.erase(str.begin(), str.begin() + 1);
// DX 20200313 [reformatted and moved to AUROSTD]    }
// DX 20200313 [reformatted and moved to AUROSTD]    if(havechar(str, '/')) {
// DX 20200313 [reformatted and moved to AUROSTD]      double numerator = 0;
// DX 20200313 [reformatted and moved to AUROSTD]      double denominator = 0;
// DX 20200313 [reformatted and moved to AUROSTD]      uint slash = whereischar(str, '/');
// DX 20200313 [reformatted and moved to AUROSTD]      for (uint i = 0; i < slash; i++)
// DX 20200313 [reformatted and moved to AUROSTD]        ss_num << str[i];
// DX 20200313 [reformatted and moved to AUROSTD]      for (uint i = 0; i < (str.size() - slash); i++)
// DX 20200313 [reformatted and moved to AUROSTD]        ss_den << str[i + slash + 1];
// DX 20200313 [reformatted and moved to AUROSTD]      string num_string = ss_num.str();
// DX 20200313 [reformatted and moved to AUROSTD]      const char* num = num_string.c_str();
// DX 20200313 [reformatted and moved to AUROSTD]      numerator = atof(num);
// DX 20200313 [reformatted and moved to AUROSTD]      string den_string = ss_den.str();
// DX 20200313 [reformatted and moved to AUROSTD]      const char* den = den_string.c_str();
// DX 20200313 [reformatted and moved to AUROSTD]      denominator = atof(den);
// DX 20200313 [reformatted and moved to AUROSTD]      if(neg == true) {
// DX 20200313 [reformatted and moved to AUROSTD]        out = -numerator / denominator;
// DX 20200313 [reformatted and moved to AUROSTD]      } else {
// DX 20200313 [reformatted and moved to AUROSTD]        out = numerator / denominator;
// DX 20200313 [reformatted and moved to AUROSTD]      }
// DX 20200313 [reformatted and moved to AUROSTD]    } else {
// DX 20200313 [reformatted and moved to AUROSTD]      char dbl[256];
// DX 20200313 [reformatted and moved to AUROSTD]      for (uint i = 0; i < str.size(); i++)
// DX 20200313 [reformatted and moved to AUROSTD]        dbl[i] = str[i];
// DX 20200313 [reformatted and moved to AUROSTD]      out = atof(dbl);
// DX 20200313 [reformatted and moved to AUROSTD]    }
// DX 20200313 [reformatted and moved to AUROSTD]    return out;
// DX 20200313 [reformatted and moved to AUROSTD]  }
// DX 20200313 [reformatted and moved to AUROSTD]}
  ...
  vector<sdouble> simplify(string str) {
    ...
    for (uint S = 0; S < splitstr.size(); S++) {
      ...
      if(!containschar(oss.str())) {
        if(havechar(oss.str(), '/')) {
          //DX 20200313 [OBSOLETE] num += frac2dbl(oss.str());
          num += aurostd::frac2dbl(oss.str()); // DX 20200313
        } else {
          num += atof(oss.str().c_str());
        }
        oss.str("");
        ...
        skip = true;
        if(havechar(oss.str(), '/')) {
          //DX 20200313 [OBSOLETE] out.dbl = frac2dbl(oss.str());
          out.dbl = aurostd::frac2dbl(oss.str()); // DX 20200313
        } else {
...
namespace SYM {
  deque<deque<_atom> > shiftSymmetryEquivalentAtoms(deque<deque<_atom> >& equivalent_atoms, xmatrix<double>& lattice, xvector<double>& translation, double& min_dist, double& tol) { //DX 20190215
    bool LDEBUG = (FALSE || XHOST.DEBUG);
...
namespace SYM {
  vector<Glide> mirror_operations(vector<xvector<double> > expanded_lattice, vector<xvector<double> > expanded_cell, xmatrix<double> L, xmatrix<double> Linv, vector<xvector<double> >& lattice_vectors, double& radius, bool& skew, double& tol) { //DX 20190215
    //GET MIRROR PLANES
    //*********************************************************************
...
// **********************************************************************************************************************
namespace SYM {
  vector<Screw> triplet_operations(vector<xvector<double> > expanded_lattice, vector<xvector<double> > expanded_cell, xmatrix<double> L, xmatrix<double> Linv, vector<xvector<double> >& lattice_vectors, double& radius, bool& skew, double& tol) { //DX 20190215
    int q = 3;
...
namespace SYM {
  vector<xvector<double> > ReturnITCGenShift(int spacegroupnum, string axis_cell) {
    ...
    int s = 0;
    vector<int> wint = get_multiplicities(ITC_sym_info.gl_sgs[spacegroupnum]); //DX 20190215
...
namespace SYM {
  char discern_rot_sym(xmatrix<double> m, double& tol) {  //m is metric_tensor //DX 20190215
    ...
    char out = 'z';
    //Check 3-fold
    if(aurostd::abs(m(1, 1) - m(2, 2)) < tol && ((m(1, 1) + m(2, 2)) / 2) * sin(aurostd::abs(m(1, 2) - Pi_r / 3)) < tol) { //DX 20190215
      out = 'a';
      return out;
    }
