...
namespace AGL_functions {
  ...
  uint extractedos(vector<_xvasp>& vaspRuns, _AGL_data& AGL_data, vector<string>& dirrunname, ofstream& FileMESSAGE) {
	...
	if (doscar.vDOS.at(0).at(0).at(i).size() < doscar.venergy.size()) {  // ME190614 - new vDOS format
	  aurostd::StringstreamClean(aus);
	...
	if (doscar.viDOS.at(i).size() < doscar.venergy.size()) {  // ME190614 - new vDOS format
	  aurostd::StringstreamClean(aus);
	  aus << _AGLSTR_WARNING_ + "Mismatch between number of DOS values and number of energy values" << endl;
	  aurostd::PrintMessageStream(FileMESSAGE,aus,XHOST.QUIET);
	  continue;
	...
	for (uint j = 0; j < doscar.spin + 1; j++) {  // ME190614 - new vDOS format
	  sumdos = sumdos + doscar.vDOS.at(0).at(0).at(j).at(i);  // ME190614 - new vDOS format
	  sumidos = sumidos + doscar.viDOS.at(j).at(i);  // ME190614 - new viDOS format
	}
