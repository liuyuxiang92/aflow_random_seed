...
// FILE "ANRL/aflow_anrl_A3B3C_hP14_176_h_h_d.cpp"

#ifndef _AFLOW_ANRL_A3B3C_hP14_176_h_h_d_CPP
#define _AFLOW_ANRL_A3B3C_hP14_176_h_h_d_CPP
#include "../aflow.h"

namespace anrl {
  uint WebANRL_A3B3C_hP14_176_h_h_d(stringstream &web,bool LDEBUG);
}

namespace anrl {
  uint PrototypeANRL_A3B3C_hP14_176_h_h_d(ostream &oss,stringstream &web,xstructure& str,string parameters,string proto_line,uint print_mode,bool LDEBUG) {
    // system A3B3C_hP14_176_h_h_d

    if(XHOST.vflag_control.flag("WWW")) {
      WebANRL_A3B3C_hP14_176_h_h_d(web,LDEBUG); // PLUG WEB STUFF
      #ifdef _ANRL_NOWEB_
      cout << "no web" << endl;
      #else
      cout << web.str() << endl;
    ...
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_A3B3C_hP14_176_h_h_d: FOUND" << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_A3B3C_hP14_176_h_h_d: label=" << label << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_A3B3C_hP14_176_h_h_d: nspecies=" << nspecies << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_A3B3C_hP14_176_h_h_d: natoms=" << natoms << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_A3B3C_hP14_176_h_h_d: spacegroup=" << spacegroup << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_A3B3C_hP14_176_h_h_d: nunderscores=" << nunderscores << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_A3B3C_hP14_176_h_h_d: nparameters=" <<  nparameters << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_A3B3C_hP14_176_h_h_d: Pearson_symbol=" << Pearson_symbol << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_A3B3C_hP14_176_h_h_d: params=" << params << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_A3B3C_hP14_176_h_h_d: Strukturbericht=" << Strukturbericht << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_A3B3C_hP14_176_h_h_d: prototype=" << prototype << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_A3B3C_hP14_176_h_h_d: dialect=" << dialect << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_A3B3C_hP14_176_h_h_d: vparameters.size()=" << vparameters.size() << endl;}
    ...
    uint i=0;
    double a=vparameters.at(i++);                  if(LDEBUG) { cerr << "anrl::PrototypeANRL_A3B3C_hP14_176_h_h_d: a=" << a << endl;}
    double covera=vparameters.at(i++),c=covera*a;  if(LDEBUG) { cerr << "anrl::PrototypeANRL_A3B3C_hP14_176_h_h_d: c=" << c << " (c/a=" << covera << ")" << endl;}
    
    double x2=vparameters.at(i++);                 if(LDEBUG) { cerr << "anrl::PrototypeANRL_A3B3C_hP14_176_h_h_d: x2=" << x2 << endl;}
    double y2=vparameters.at(i++);                 if(LDEBUG) { cerr << "anrl::PrototypeANRL_A3B3C_hP14_176_h_h_d: y2=" << y2 << endl;}
    double x3=vparameters.at(i++);                 if(LDEBUG) { cerr << "anrl::PrototypeANRL_A3B3C_hP14_176_h_h_d: x3=" << x3 << endl;}
    double y3=vparameters.at(i++);                 if(LDEBUG) { cerr << "anrl::PrototypeANRL_A3B3C_hP14_176_h_h_d: y3=" << y3 << endl;}
    ...
    atom.fpos(1)=(2.0/3.0);atom.fpos(2)=(1.0/3.0);atom.fpos(3)=(1.0/4.0);                     // atom B1
    atom.fpos_equation.clear();atom.fpos_equation.push_back("(2.0/3.0)");atom.fpos_equation.push_back("(1.0/3.0)");atom.fpos_equation.push_back("(1.0/4.0)");// atom B1 // symbolic math for atom positions
    str.comp_each_type.at(atom.type)+=1.0;                            // atom B1 // if we need partial occupation
    str.atoms.push_back(atom);                                        // atom B1
    
    atom.name="C"; atom.type=2;                                       // atom B2
    atom.fpos(1)=(1.0/3.0);atom.fpos(2)=(2.0/3.0);atom.fpos(3)=(3.0/4.0);                     // atom B2
    atom.fpos_equation.clear();atom.fpos_equation.push_back("(1.0/3.0)");atom.fpos_equation.push_back("(2.0/3.0)");atom.fpos_equation.push_back("(3.0/4.0)");// atom B2 // symbolic math for atom positions
...
namespace anrl {
  uint WebANRL_A3B3C_hP14_176_h_h_d(stringstream& web,bool LDEBUG) {
    ...
    if(LDEBUG) {cerr << "anrl:: WebANRL_A3B3C_hP14_176_h_h_d: web.str().size()=" << web.str().size() << endl;}
