...
// FILE "ANRL/aflow_anrl_A_cF136_227_aeg.cpp"
...
namespace anrl {
  uint PrototypeANRL_A_cF136_227_aeg(ostream &oss,stringstream &web,xstructure& str,string parameters,string proto_line,uint print_mode,bool LDEBUG) {
    ...
    atom.fpos_equation.clear();atom.fpos_equation.push_back("x2");atom.fpos_equation.push_back("x2");atom.fpos_equation.push_back("((1.0/2.0)-3.0*x2)");// atom B4 // symbolic math for atom positions
    str.comp_each_type.at(atom.type)+=1.0;                            // atom B4 // if we need partial occupation
    str.atoms.push_back(atom);                                        // atom B4
    
    atom.name="A"; atom.type=0;                                       // atom B5
    atom.fpos(1)=x2;atom.fpos(2)=((1.0/2.0)-3.0*x2);atom.fpos(3)=x2;                     // atom B5
    atom.fpos_equation.clear();atom.fpos_equation.push_back("x2");atom.fpos_equation.push_back("((1.0/2.0)-3.0*x2)");atom.fpos_equation.push_back("x2");// atom B5 // symbolic math for atom positions
    str.comp_each_type.at(atom.type)+=1.0;                            // atom B5 // if we need partial occupation
    str.atoms.push_back(atom);                                        // atom B5
    
    atom.name="A"; atom.type=0;                                       // atom B6
    atom.fpos(1)=((1.0/2.0)-3.0*x2);atom.fpos(2)=x2;atom.fpos(3)=x2;                     // atom B6
    atom.fpos_equation.clear();atom.fpos_equation.push_back("((1.0/2.0)-3.0*x2)");atom.fpos_equation.push_back("x2");atom.fpos_equation.push_back("x2");// atom B6 // symbolic math for atom positions
    str.comp_each_type.at(atom.type)+=1.0;                            // atom B6 // if we need partial occupation
    str.atoms.push_back(atom);                                        // atom B6
    
    atom.name="A"; atom.type=0;                                       // atom B7
    atom.fpos(1)=-x2;atom.fpos(2)=-x2;atom.fpos(3)=((1.0/2.0)+3.0*x2);                     // atom B7
    atom.fpos_equation.clear();atom.fpos_equation.push_back("-x2");atom.fpos_equation.push_back("-x2");atom.fpos_equation.push_back("((1.0/2.0)+3.0*x2)");// atom B7 // symbolic math for atom positions
    ...
    atom.fpos_equation.clear();atom.fpos_equation.push_back("-x2");atom.fpos_equation.push_back("((1.0/2.0)+3.0*x2)");atom.fpos_equation.push_back("-x2");// atom B9 // symbolic math for atom positions
    str.comp_each_type.at(atom.type)+=1.0;                            // atom B9 // if we need partial occupation
    str.atoms.push_back(atom);                                        // atom B9
    
    atom.name="A"; atom.type=0;                                       // atom B10
    atom.fpos(1)=((1.0/2.0)+3.0*x2);atom.fpos(2)=-x2;atom.fpos(3)=-x2;                     // atom B10
    atom.fpos_equation.clear();atom.fpos_equation.push_back("((1.0/2.0)+3.0*x2)");atom.fpos_equation.push_back("-x2");atom.fpos_equation.push_back("-x2");// atom B10 // symbolic math for atom positions
    str.comp_each_type.at(atom.type)+=1.0;                            // atom B10 // if we need partial occupation
    str.atoms.push_back(atom);                                        // atom B10
    
    atom.name="A"; atom.type=0;                                       // atom B11
    atom.fpos(1)=z3;atom.fpos(2)=z3;atom.fpos(3)=(2.0*x3-z3);                     // atom B11
    atom.fpos_equation.clear();atom.fpos_equation.push_back("z3");atom.fpos_equation.push_back("z3");atom.fpos_equation.push_back("(2.0*x3-z3)");// atom B11 // symbolic math for atom positions
    str.comp_each_type.at(atom.type)+=1.0;                            // atom B11 // if we need partial occupation
    str.atoms.push_back(atom);                                        // atom B11
    
    atom.name="A"; atom.type=0;                                       // atom B12
    atom.fpos(1)=z3;atom.fpos(2)=z3;atom.fpos(3)=((1.0/2.0)-2.0*x3-z3);                     // atom B12
    atom.fpos_equation.clear();atom.fpos_equation.push_back("z3");atom.fpos_equation.push_back("z3");atom.fpos_equation.push_back("((1.0/2.0)-2.0*x3-z3)");// atom B12 // symbolic math for atom positions
    ...
    atom.fpos_equation.clear();atom.fpos_equation.push_back("(2.0*x3-z3)");atom.fpos_equation.push_back("((1.0/2.0)-2.0*x3-z3)");atom.fpos_equation.push_back("z3");// atom B13 // symbolic math for atom positions
    ...
    atom.fpos_equation.clear();atom.fpos_equation.push_back("((1.0/2.0)-2.0*x3-z3)");atom.fpos_equation.push_back("(2.0*x3-z3)");atom.fpos_equation.push_back("z3");// atom B14 // symbolic math for atom positions
    str.comp_each_type.at(atom.type)+=1.0;                            // atom B14 // if we need partial occupation
    str.atoms.push_back(atom);                                        // atom B14
    
    atom.name="A"; atom.type=0;                                       // atom B15
    atom.fpos(1)=(2.0*x3-z3);atom.fpos(2)=z3;atom.fpos(3)=z3;                     // atom B15
    atom.fpos_equation.clear();atom.fpos_equation.push_back("(2.0*x3-z3)");atom.fpos_equation.push_back("z3");atom.fpos_equation.push_back("z3");// atom B15 // symbolic math for atom positions
    str.comp_each_type.at(atom.type)+=1.0;                            // atom B15 // if we need partial occupation
    str.atoms.push_back(atom);                                        // atom B15
    
    atom.name="A"; atom.type=0;                                       // atom B16
    atom.fpos(1)=((1.0/2.0)-2.0*x3-z3);atom.fpos(2)=z3;atom.fpos(3)=z3;                     // atom B16
    atom.fpos_equation.clear();atom.fpos_equation.push_back("((1.0/2.0)-2.0*x3-z3)");atom.fpos_equation.push_back("z3");atom.fpos_equation.push_back("z3");// atom B16 // symbolic math for atom positions
    ...
    atom.fpos_equation.clear();atom.fpos_equation.push_back("z3");atom.fpos_equation.push_back("(2.0*x3-z3)");atom.fpos_equation.push_back("((1.0/2.0)-2.0*x3-z3)");// atom B17 // symbolic math for atom positions
    ...
    atom.fpos_equation.clear();atom.fpos_equation.push_back("z3");atom.fpos_equation.push_back("((1.0/2.0)-2.0*x3-z3)");atom.fpos_equation.push_back("(2.0*x3-z3)");// atom B18 // symbolic math for atom positions
    str.comp_each_type.at(atom.type)+=1.0;                            // atom B18 // if we need partial occupation
    str.atoms.push_back(atom);                                        // atom B18
    
    atom.name="A"; atom.type=0;                                       // atom B19
    atom.fpos(1)=z3;atom.fpos(2)=(2.0*x3-z3);atom.fpos(3)=z3;                     // atom B19
    atom.fpos_equation.clear();atom.fpos_equation.push_back("z3");atom.fpos_equation.push_back("(2.0*x3-z3)");atom.fpos_equation.push_back("z3");// atom B19 // symbolic math for atom positions
    str.comp_each_type.at(atom.type)+=1.0;                            // atom B19 // if we need partial occupation
    str.atoms.push_back(atom);                                        // atom B19
    
    atom.name="A"; atom.type=0;                                       // atom B20
    atom.fpos(1)=z3;atom.fpos(2)=((1.0/2.0)-2.0*x3-z3);atom.fpos(3)=z3;                     // atom B20
    atom.fpos_equation.clear();atom.fpos_equation.push_back("z3");atom.fpos_equation.push_back("((1.0/2.0)-2.0*x3-z3)");atom.fpos_equation.push_back("z3");// atom B20 // symbolic math for atom positions
    ...
    atom.fpos_equation.clear();atom.fpos_equation.push_back("((1.0/2.0)-2.0*x3-z3)");atom.fpos_equation.push_back("z3");atom.fpos_equation.push_back("(2.0*x3-z3)");// atom B21 // symbolic math for atom positions
    ...
    atom.fpos_equation.clear();atom.fpos_equation.push_back("(2.0*x3-z3)");atom.fpos_equation.push_back("z3");atom.fpos_equation.push_back("((1.0/2.0)-2.0*x3-z3)");// atom B22 // symbolic math for atom positions
    str.comp_each_type.at(atom.type)+=1.0;                            // atom B22 // if we need partial occupation
    str.atoms.push_back(atom);                                        // atom B22
    
    atom.name="A"; atom.type=0;                                       // atom B23
    atom.fpos(1)=-z3;atom.fpos(2)=-z3;atom.fpos(3)=((1.0/2.0)+2.0*x3+z3);                     // atom B23
    atom.fpos_equation.clear();atom.fpos_equation.push_back("-z3");atom.fpos_equation.push_back("-z3");atom.fpos_equation.push_back("((1.0/2.0)+2.0*x3+z3)");// atom B23 // symbolic math for atom positions
    str.comp_each_type.at(atom.type)+=1.0;                            // atom B23 // if we need partial occupation
    str.atoms.push_back(atom);                                        // atom B23
    
    atom.name="A"; atom.type=0;                                       // atom B24
    atom.fpos(1)=-z3;atom.fpos(2)=-z3;atom.fpos(3)=(z3-2.0*x3);                     // atom B24
    atom.fpos_equation.clear();atom.fpos_equation.push_back("-z3");atom.fpos_equation.push_back("-z3");atom.fpos_equation.push_back("(z3-2.0*x3)");// atom B24 // symbolic math for atom positions
    ...
    atom.fpos_equation.clear();atom.fpos_equation.push_back("(z3-2.0*x3)");atom.fpos_equation.push_back("((1.0/2.0)+2.0*x3+z3)");atom.fpos_equation.push_back("-z3");// atom B25 // symbolic math for atom positions
    ...
    atom.fpos_equation.clear();atom.fpos_equation.push_back("((1.0/2.0)+2.0*x3+z3)");atom.fpos_equation.push_back("(z3-2.0*x3)");atom.fpos_equation.push_back("-z3");// atom B26 // symbolic math for atom positions
    ...
    atom.fpos_equation.clear();atom.fpos_equation.push_back("(z3-2.0*x3)");atom.fpos_equation.push_back("-z3");atom.fpos_equation.push_back("((1.0/2.0)+2.0*x3+z3)");// atom B27 // symbolic math for atom positions
    ...
    atom.fpos_equation.clear();atom.fpos_equation.push_back("((1.0/2.0)+2.0*x3+z3)");atom.fpos_equation.push_back("-z3");atom.fpos_equation.push_back("(z3-2.0*x3)");// atom B28 // symbolic math for atom positions
    ...
    atom.fpos_equation.clear();atom.fpos_equation.push_back("-z3");atom.fpos_equation.push_back("(z3-2.0*x3)");atom.fpos_equation.push_back("-z3");// atom B29 // symbolic math for atom positions
    ...
    atom.fpos_equation.clear();atom.fpos_equation.push_back("-z3");atom.fpos_equation.push_back("((1.0/2.0)+2.0*x3+z3)");atom.fpos_equation.push_back("-z3");// atom B30 // symbolic math for atom positions
    ...
    atom.fpos_equation.clear();atom.fpos_equation.push_back("-z3");atom.fpos_equation.push_back("(z3-2.0*x3)");atom.fpos_equation.push_back("((1.0/2.0)+2.0*x3+z3)");// atom B31 // symbolic math for atom positions
    ...
    atom.fpos_equation.clear();atom.fpos_equation.push_back("-z3");atom.fpos_equation.push_back("((1.0/2.0)+2.0*x3+z3)");atom.fpos_equation.push_back("(z3-2.0*x3)");// atom B32 // symbolic math for atom positions
    ...
    atom.fpos_equation.clear();atom.fpos_equation.push_back("((1.0/2.0)+2.0*x3+z3)");atom.fpos_equation.push_back("-z3");atom.fpos_equation.push_back("-z3");// atom B33 // symbolic math for atom positions
    ...
    atom.fpos_equation.clear();atom.fpos_equation.push_back("(z3-2.0*x3)");atom.fpos_equation.push_back("-z3");atom.fpos_equation.push_back("-z3");// atom B34 // symbolic math for atom positions
