...
namespace ALIEN {
  bool Produce_INPUT_FILE(_xalien& xalien,string AflowIn,ifstream &FileAFLOWIN,ofstream &FileMESSAGE,_aflags &aflags,_kflags &kflags,_alienflags &alienflags) {        // AFLOW_FUNCTION_IMPLEMENTATION
    ...
    if(!Krun)  {
      ...
      aus << "AAAAA  ALIEN, no input file specified "  << Message(aflags,_AFLOW_MESSAGE_DEFAULTS_,_AFLOW_FILE_NAME_) << endl;
    ...
    if(Krun && alienflags.KBIN_ALIEN_INPUT_MODE_IMPLICIT) {
      aus << "EEEEE  [ALIEN_INPUT_FILE_IMPLICIT] is not supporded      "  << Message(aflags,_AFLOW_MESSAGE_DEFAULTS_,_AFLOW_FILE_NAME_) << endl;
      aus << "EEEEE  [ALIEN_INPUT_FILE_EXPLICIT] is supported mode     "  << Message(aflags,_AFLOW_MESSAGE_DEFAULTS_,_AFLOW_FILE_NAME_) << endl;
      aus << "EEEEE  [ALIEN_INPUT_FILE_EXTERNAL] is supported mode     "  << Message(aflags,_AFLOW_MESSAGE_DEFAULTS_,_AFLOW_FILE_NAME_) << endl;
      aurostd::PrintErrorStream(FileMESSAGE,aus,XHOST.QUIET);    
...
namespace ALIEN {
  bool Modify_INPUT_FILE(_xalien& xalien,ofstream &FileMESSAGE,_aflags &aflags,_alienflags &alienflags) {        // AFLOW_FUNCTION_IMPLEMENTATION
    ...
    //   if(xalien.INPUT_generated==FALSE) {
    //     aus << "EEEEE  ALIEN::Modify_INPUT: can`t modify INPUT if it does not exist ! "  << Message(aflags,_AFLOW_MESSAGE_DEFAULTS_,_AFLOW_FILE_NAME_) << endl;
    //     aurostd::PrintErrorStream(FileMESSAGE,aus,XHOST.QUIET);    
    //     Krun=FALSE;
    //     return Krun;
    //   }
