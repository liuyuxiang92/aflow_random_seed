  ...
  template<class utype>
    ...
    utype* xmatrix<utype>::operator[] (int ir) const {
      ...
      if(ir>urows)  {
        ...
        throw xerror(_AFLOW_FILE_NAME_, __func__, message, _INDEX_BOUNDS_);
      }
      if(ir<lrows) {
        stringstream message;
  ...
  template<class utype> xmatrix<utype>&
    ...
    xmatrix<utype>::operator +=(const xmatrix<utype>& r)
    {
      ...
      if(this->rows!=r.rows||this->cols!=r.cols) {
        ...
        throw xerror(_AFLOW_FILE_NAME_, __func__, message, _INDEX_MISMATCH_);
      }
  ...
  template<class utype> bool
    identical(const xmatrix<xcomplex<utype> >& a, const xmatrix<xcomplex<utype> >& b, const utype& _tol_, const char& _mode_) {
      ...
      if (_mode_ == 1) {  // relative tolerance
        ...
        throw xerror(_AFLOW_FILE_NAME_, __func__, message, _VALUE_ILLEGAL_);
      }
      return true;
    }
  ...
  template<class utype>                                    // GaussJordan xmatrix
    void GaussJordan(xmatrix<utype>& A, xmatrix<utype>& B) {
      ...
      if(A.lrows!=1) {
        ...
        throw xerror(_AFLOW_FILE_NAME_, __func__, message, _INDEX_ERROR_);
      }
      if(A.lcols!=1) {
...
namespace aurostd {   // least square stuff aurostd adaptation of nrecipes    // 1 August 2014
  ...
  template<class utype> void gaussj(xmatrix<utype>& a, int n, xmatrix<utype>& b, int m) {  // with indices
    ...
    if(n>a.rows) {
      message = "n>a.rows";
      throw xerror(_AFLOW_FILE_NAME_, __func__, message, _VALUE_RANGE_);
    }
    if(n>b.rows) {
      message = "n>b.rows";
      throw xerror(_AFLOW_FILE_NAME_, __func__, message, _VALUE_RANGE_);
...
namespace aurostd {
  template<class utype> void polarDecomposition(const xmatrix<utype>& transformation_matrix,
    ...
    if(!transformation_matrix.issquare){
      ...
      throw xerror(_AFLOW_FILE_NAME_, __func__, message, _INPUT_ERROR_);
    }
    ...
    if(LDEBUG){ cerr << __func__ << " T^2: " << T_squared << endl; }
    ...
    if(LDEBUG){
      cerr << __func__ << " diag: " << diag << endl;
      cerr << __func__ << " eigen_vec: " << eigen_vec << endl;
    }
    ...
    if(check_orthogonal_rotation){
      ...
      if(LDEBUG){
        // R^T==R^-1
        cerr << __func__ << " transpose(R):" << endl << aurostd::trasp(rotation) << endl;
        cerr << __func__ << " inverse(R):" << endl << aurostd::inverse(rotation) << endl;
        // R*R^T=I
        cerr << __func__ << " identity? (R*R^T=I):" << endl << identity_matrix << endl;
      }
