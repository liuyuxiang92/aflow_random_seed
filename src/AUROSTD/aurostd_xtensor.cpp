...
namespace aurostd{
  ...
  template<class utype>
    _subtensor<utype>& _subtensor<utype>::operator[] (const int& i) {
      ...
      std::string function_name = _SUBTENSOR_ERR_PREFIX_ + "operator[]";
...
namespace aurostd {
  ...
  template<class utype>
    void xtensor<utype>::buildTensor(const std::vector<int>& uind, const std::vector<int>& lind) {
      ...
      if (checkInit(uind, lind)) {
        ...
        if (!corpus) {
          std::string function_name = _XTENSOR_ERR_PREFIX_ + "xtensor";
          std::string message = "Allocation failure in xtensor constructor.";
          throw xerror(_AFLOW_FILE_NAME_,function_name, message, _ALLOC_ALLOCATE_);
        }
        // Exception handling
      } else {
        ...
        throw xerror(_AFLOW_FILE_NAME_,function_name, message, code);
      }
    }
...
#ifdef _CHECK_BOUNDS_
      std::string function_name = _XTENSOR_ERR_PREFIX_ + "operator()";
      std::stringstream message;
  ...
  template<class utype>
    std::vector<utype> xtensor2vector(xtensor<utype>& tensor) {
      ...
      if (tensor.ndim > 1) {
        // Exception handling
        std::string function_name = _XTENSOR_ERR_PREFIX_ + "xtensor2vector";
        std::stringstream message;
  ...
  template<class utype>
    xvector<utype> xtensor2xvector(xtensor<utype>& tensor) {
      ...
      if (tensor.ndim > 1) {
        // Exception handling
        std::string function_name = _XTENSOR_ERR_PREFIX_ + "xtensor2xvector";
        std::stringstream message;
  ...
  template<class utype>
    xmatrix<utype> xtensor2xmatrix(xtensor<utype>& tensor) {
      ...
      if (tensor.ndim > 2) {
        // Exception handling
        std::string function_name = _XTENSOR_ERR_PREFIX_ + "xtensor2xmatrix";
        std::stringstream message;
  ...
  template<class utype>
    utype trace(const xtensor<utype>& tensor) {
      ...
      if (tensor.is_cubic) {
        ...
        std::string function_name = _XTENSOR_ERR_PREFIX_ + "trace";
