...
template<class utype> bool initialize_xscalar_xvector_xmatrix_xtensor(utype x) {
  ...
  vv=m.getcol(1);vv=m.getdiag(0,1);m.setrow(v);m.setcol(v);m.setmat(n);m.setmat(v);m=n;m=m+n;m=m-n;m=m*n;m=inverseByMinors(m);m=inverse(m);isNonInvertible(m);m=reduce_to_shortest_basis(m);		//CO190329 - clang doesn't like x=x, changing to x=y  //CO191110  //CO191201
