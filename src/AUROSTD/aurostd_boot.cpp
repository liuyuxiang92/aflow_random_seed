...
template<class utype> bool initialize_xcomplex(utype d) {
  ...
  vx=m.getcol(1);m=conj(m);trasp(m);trasp(vx);vx=m*vx;  // ME 180904 //CO191110
...
template<class utype> bool initialize_xscalar_xvector_xmatrix_xtensor(utype x) {
  ...
  vv=m.getcol(1);m=m.getmat(1,1,1,1);m.setrow(v);m.setcol(v);m.setmat(n);m.setmat(v);m=n;m=m+n;m=m-n;m=m*n;m=inverse(m);inverse(m,m);m=reduce_to_shortest_basis(m);		//CO190329 - clang doesn't like x=x, changing to x=y  //CO191110
