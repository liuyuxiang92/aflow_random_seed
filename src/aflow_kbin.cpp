...
#define MACHINE005_DEFAULT_KILL_MEM_CUTOFF 1.50  //CO20230512 - MACHINE005
#define MACHINE006_DEFAULT_KILL_MEM_CUTOFF 1.50  //CO20230512 - MACHINE006
...
namespace KBIN {
  int KBIN_Main(vector<string> argv) {        // AFLOW_FUNCTION_IMPLEMENTATION
    ...
    // "MACHINE::MACHINE005"
    aflags.AFLOW_MACHINE_GLOBAL.flag("MACHINE::MACHINE005",aurostd::args2flag(argv,"--machine=machine005"));
    if(aflags.AFLOW_MACHINE_GLOBAL.flag("MACHINE::MACHINE005")) XHOST.maxmem=MACHINE005_DEFAULT_KILL_MEM_CUTOFF;
    //CO20230512 - MACHINE005 - END
    //CO20230512 - MACHINE006 - START
    // "MACHINE::MACHINE006"
    aflags.AFLOW_MACHINE_GLOBAL.flag("MACHINE::MACHINE006",aurostd::args2flag(argv,"--machine=machine006"));
    if(aflags.AFLOW_MACHINE_GLOBAL.flag("MACHINE::MACHINE006")) XHOST.maxmem=MACHINE006_DEFAULT_KILL_MEM_CUTOFF;
  ...
  void MPI_Extract(string AflowIn,ofstream &FileMESSAGE,_aflags &aflags,_kflags &kflags) {
    ...
    if(aflags.AFLOW_GLOBAL_NCPUS<1) {
      ...
      if(Kmpi && (aurostd::substring2bool(AflowIn,"[AFLOW_MODE_MPI_MODE]NCPUS=MAX",TRUE) || aurostd::substring2bool(AflowIn,"[AFLOW_MODE_MPI_MODE]NCPUS=AUTO",TRUE) || aflags.AFLOW_MACHINE_LOCAL.flag())) { // DEFAULT NCPUS=MAX
        ...
        if(aflags.AFLOW_MACHINE_LOCAL.flag("MACHINE::MACHINE005")) kflags.KBIN_MPI_NCPUS=XHOST.PBS_NUM_PPN;   // with MACHINE005; //CO20230512
        if(aflags.AFLOW_MACHINE_LOCAL.flag("MACHINE::MACHINE006")){   // with MACHINE006; //CO20230512
...
namespace KBIN {
  void RUN_Directory(_aflags& aflags) {        // AFLOW_FUNCTION_IMPLEMENTATION
    ...
    if(aflags.AFLOW_PERFORM_FILE && ( //CO20230528 - only run in a different directory if aflags.AFLOW_PERFORM_FILE, we should be benchmarking non-HT
        ...
        aflags.AFLOW_MACHINE_GLOBAL.flag("MACHINE::MACHINE006") ||  //CO20230512
