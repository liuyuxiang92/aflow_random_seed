...
#define MACHINE105_DEFAULT_KILL_MEM_CUTOFF 1.50  //CO20230512 - MACHINE105
#define MACHINE106_DEFAULT_KILL_MEM_CUTOFF 1.50  //CO20230512 - MACHINE106
...
namespace KBIN {
  int KBIN_Main(vector<string> argv) {        // AFLOW_FUNCTION_IMPLEMENTATION
    ...
    // "MACHINE::MACHINE105"
    aflags.AFLOW_MACHINE_GLOBAL.flag("MACHINE::MACHINE105",aurostd::args2flag(argv,"--machine=machine005"));
    if(aflags.AFLOW_MACHINE_GLOBAL.flag("MACHINE::MACHINE105")) XHOST.maxmem=MACHINE105_DEFAULT_KILL_MEM_CUTOFF;
    //CO20230512 - MACHINE105 - END
    //CO20230512 - MACHINE106 - START
    // "MACHINE::MACHINE106"
    aflags.AFLOW_MACHINE_GLOBAL.flag("MACHINE::MACHINE106",aurostd::args2flag(argv,"--machine=machine006"));
    if(aflags.AFLOW_MACHINE_GLOBAL.flag("MACHINE::MACHINE106")) XHOST.maxmem=MACHINE106_DEFAULT_KILL_MEM_CUTOFF;
  ...
  void MPI_Extract(string AflowIn,ofstream &FileMESSAGE,_aflags &aflags,_kflags &kflags) {
    ...
    if(aflags.AFLOW_GLOBAL_NCPUS<1) {
      ...
      if(Kmpi && (aurostd::substring2bool(AflowIn,"[AFLOW_MODE_MPI_MODE]NCPUS=MAX",TRUE) || aurostd::substring2bool(AflowIn,"[AFLOW_MODE_MPI_MODE]NCPUS=AUTO",TRUE) || aflags.AFLOW_MACHINE_LOCAL.flag())) { // DEFAULT NCPUS=MAX
        ...
        if(aflags.AFLOW_MACHINE_LOCAL.flag("MACHINE::MACHINE105")) kflags.KBIN_MPI_NCPUS=XHOST.PBS_NUM_PPN;   // with MACHINE105; //CO20230512
        if(aflags.AFLOW_MACHINE_LOCAL.flag("MACHINE::MACHINE106")){   // with MACHINE106; //CO20230512
...
namespace KBIN {
  void RUN_Directory(_aflags& aflags) {        // AFLOW_FUNCTION_IMPLEMENTATION
    ...
    if(aflags.AFLOW_PERFORM_FILE && ( //CO20230528 - only run in a different directory if aflags.AFLOW_PERFORM_FILE, we should be benchmarking non-HT
        ...
        aflags.AFLOW_MACHINE_GLOBAL.flag("MACHINE::MACHINE105") ||  //CO20230512
        aflags.AFLOW_MACHINE_GLOBAL.flag("MACHINE::MACHINE106") ||  //CO20230512
