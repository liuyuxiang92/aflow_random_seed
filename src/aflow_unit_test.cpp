...
namespace unittest {
 ...
 //   checkEqual(calculated_xvecint1, expected_xvecint1, check_function, check_description1, passed_checks, results);
    full_xmatint.getmatInPlace(expected_xvecint,1,3,1,1,1,1);
    cerr << expected_xvecint << endl;
    calculated_xvecint = full_xmatint.getmat(1,3,1,1).best_getvec();
    cerr << calculated_xvecint << endl;

    calculated_xvecint1 = full_xmatint.best_getvec(1,3,1,1);    
    cerr << calculated_xvecint1 << endl;
