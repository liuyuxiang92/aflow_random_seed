...
namespace aflowML {
  ...
  void insertElementalPropertiesCoordCorrEnth(const vector<string>& vproperties,
  ...
  void writeCoordCorrEnthCSV() {
    bool LDEBUG=(TRUE || _DEBUG_ML_ || XHOST.DEBUG);
    string soliloquy=XPID+"aflowML::writeCoordCorrEnthCSV():";
    ...
    vheaders.insert(vheaders.end(),vheaders_additional.begin(),vheaders_additional.end());
    uint count_vcols_ecombo_coord_corr_enth=vheaders_additional.size();
    ...
    for(ielement=0;ielement<100;ielement++){
      for(ioxidation=0;ioxidation<10;ioxidation++){
        ...
        if(!correction_line.empty()){
          ...
          //
          insertElementalPropertiesCoordCorrEnth(vproperties_elements,xel_cation,M_X_bonds,natoms_per_fu_cation,vitems);
          insertElementalPropertiesCoordCorrEnth(vproperties_elements,xel_N,M_X_bonds,natoms_per_fu_anion,vitems);
          //
          insertCrystalProperties(structure_path,"N",vheaders,vitems,entry,eproperties_full);
          //
          insertElementalCombinations(vproperties_elements_numbers,xel_cation,xel_N,entry,M_X_bonds,natoms_per_fu_cation,natoms_per_fu_anion,vheaders_additional,vfeatures,false,count_vcols_ecombo_coord_corr_enth);
        ...
        if(!correction_line.empty()){
          ...
          //
          insertElementalPropertiesCoordCorrEnth(vproperties_elements,xel_cation,M_X_bonds,natoms_per_fu_cation,vitems);
          insertElementalPropertiesCoordCorrEnth(vproperties_elements,xel_O,M_X_bonds,natoms_per_fu_anion,vitems);
          //
          insertCrystalProperties(structure_path,"O",vheaders,vitems,entry,eproperties_full);
          //
          insertElementalCombinations(vproperties_elements_numbers,xel_cation,xel_O,entry,M_X_bonds,natoms_per_fu_cation,natoms_per_fu_anion,vheaders_additional,vfeatures,false,count_vcols_ecombo_coord_corr_enth);
          for(i=0;i<vfeatures.size();i++){vitems.push_back(aurostd::utype2string(vfeatures[i],_DOUBLE_WRITE_PRECISION_));}
          //
    ...
    aurostd::stringstream2file(file,"coord_corr_enth_data_N.csv");
    //O
    aurostd::StringstreamClean(file);
    for(i=0;i<vvlines[1].size();i++){file << aurostd::joinWDelimiter(vvlines[1][i],",") << endl;}
    aurostd::stringstream2file(file,"coord_corr_enth_data_O.csv");
    //total
    if(0){  //we've removed anion properties in reduceFeatures()
      ...
      aurostd::stringstream2file(file,"coord_corr_enth_data_NO.csv");
    }
