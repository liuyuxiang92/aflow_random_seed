...
// FILE "ANRL/aflow_anrl_AB_oP8_33_a_a.cpp"

#ifndef _AFLOW_ANRL_AB_oP8_33_a_a_CPP // AFLOW_REMOVE_GREP
#define _AFLOW_ANRL_AB_oP8_33_a_a_CPP // AFLOW_REMOVE_GREP
#include "../aflow.h" // AFLOW_REMOVE_GREP

namespace anrl {
  uint WebANRL_AB_oP8_33_a_a(stringstream &web,bool LDEBUG);
}

namespace anrl {
  uint PrototypeANRL_AB_oP8_33_a_a(stringstream &web,xstructure& str,string parameters,string proto_line,uint print_mode,bool LDEBUG) {
    // system AB_oP8_33_a_a

    if(XHOST.vflag_control.flag("WWW")) {
      WebANRL_AB_oP8_33_a_a(web,LDEBUG); // PLUG WEB STUFF
#ifdef _ANRL_NOWEB_
      cout << "no web" << endl;
#else
      cout << web.str() << endl;
    ...
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_oP8_33_a_a: FOUND" << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_oP8_33_a_a: label=" << label << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_oP8_33_a_a: nspecies=" << nspecies << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_oP8_33_a_a: natoms=" << natoms << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_oP8_33_a_a: spacegroup=" << spacegroup << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_oP8_33_a_a: nunderscores=" << nunderscores << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_oP8_33_a_a: nparameters=" <<  nparameters << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_oP8_33_a_a: Pearson_symbol=" << Pearson_symbol << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_oP8_33_a_a: params=" << params << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_oP8_33_a_a: Strukturbericht=" << Strukturbericht << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_oP8_33_a_a: prototype=" << prototype << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_oP8_33_a_a: dialect=" << dialect << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_oP8_33_a_a: vparameters.size()=" << vparameters.size() << endl;}
    ...
    uint i=0;
    double a=vparameters.at(i++);                  if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_oP8_33_a_a: a=" << a << endl;}
    double bovera=vparameters.at(i++),b=bovera*a;  if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_oP8_33_a_a: b=" << b << " (b/a=" << bovera << ")" << endl;}
    double covera=vparameters.at(i++),c=covera*a;  if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_oP8_33_a_a: c=" << c << " (c/a=" << covera << ")" << endl;}

    double x1=vparameters.at(i++);                 if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_oP8_33_a_a: x1=" << x1 << endl;}
    double y1=vparameters.at(i++);                 if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_oP8_33_a_a: y1=" << y1 << endl;}
    double z1=vparameters.at(i++);                 if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_oP8_33_a_a: z1=" << z1 << endl;}
    double x2=vparameters.at(i++);                 if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_oP8_33_a_a: x2=" << x2 << endl;}
    double y2=vparameters.at(i++);                 if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_oP8_33_a_a: y2=" << y2 << endl;}
    double z2=vparameters.at(i++);                 if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_oP8_33_a_a: z2=" << z2 << endl;}
...
namespace anrl {
  uint WebANRL_AB_oP8_33_a_a(stringstream& web,bool LDEBUG) {
    ...
    if(LDEBUG) {cerr << "anrl:: WebANRL_AB_oP8_33_a_a: web.str().size()=" << web.str().size() << endl;}
