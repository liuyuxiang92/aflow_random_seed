...
// FILE "ANRL/aflow_anrl_AB_hP6_191_f_ad.cpp"

#ifndef _AFLOW_ANRL_AB_hP6_191_f_ad_CPP // AFLOW_REMOVE_GREP
#define _AFLOW_ANRL_AB_hP6_191_f_ad_CPP // AFLOW_REMOVE_GREP
#include "../aflow.h" // AFLOW_REMOVE_GREP

namespace anrl {
  uint WebANRL_AB_hP6_191_f_ad(stringstream &web,bool LDEBUG);
}

namespace anrl {
  uint PrototypeANRL_AB_hP6_191_f_ad(stringstream &web,xstructure& str,string parameters,string proto_line,uint print_mode,bool LDEBUG) {
    // system AB_hP6_191_f_ad

    if(XHOST.vflag_control.flag("WWW")) {
      WebANRL_AB_hP6_191_f_ad(web,LDEBUG); // PLUG WEB STUFF
#ifdef _ANRL_NOWEB_
      cout << "no web" << endl;
#else
      cout << web.str() << endl;
    ...
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_hP6_191_f_ad: FOUND" << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_hP6_191_f_ad: label=" << label << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_hP6_191_f_ad: nspecies=" << nspecies << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_hP6_191_f_ad: natoms=" << natoms << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_hP6_191_f_ad: spacegroup=" << spacegroup << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_hP6_191_f_ad: nunderscores=" << nunderscores << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_hP6_191_f_ad: nparameters=" <<  nparameters << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_hP6_191_f_ad: Pearson_symbol=" << Pearson_symbol << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_hP6_191_f_ad: params=" << params << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_hP6_191_f_ad: Strukturbericht=" << Strukturbericht << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_hP6_191_f_ad: prototype=" << prototype << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_hP6_191_f_ad: dialect=" << dialect << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_hP6_191_f_ad: vparameters.size()=" << vparameters.size() << endl;}
    ...
    uint i=0;
    double a=vparameters.at(i++);                  if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_hP6_191_f_ad: a=" << a << endl;}
    double covera=vparameters.at(i++),c=covera*a;  if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_hP6_191_f_ad: c=" << c << " (c/a=" << covera << ")" << endl;}
...
namespace anrl {
  uint WebANRL_AB_hP6_191_f_ad(stringstream& web,bool LDEBUG) {
    ...
    if(LDEBUG) {cerr << "anrl:: WebANRL_AB_hP6_191_f_ad: web.str().size()=" << web.str().size() << endl;}
