...
// FILE "ANRL/aflow_anrl_AB3C3_cF112_227_c_de_f.cpp"

#ifndef _AFLOW_ANRL_AB3C3_cF112_227_c_de_f_CPP // AFLOW_REMOVE_GREP
#define _AFLOW_ANRL_AB3C3_cF112_227_c_de_f_CPP // AFLOW_REMOVE_GREP
#include "../aflow.h" // AFLOW_REMOVE_GREP

namespace anrl {
  uint WebANRL_AB3C3_cF112_227_c_de_f(stringstream &web,bool LDEBUG);
}

namespace anrl {
  uint PrototypeANRL_AB3C3_cF112_227_c_de_f(stringstream &web,xstructure& str,string parameters,string proto_line,uint print_mode,bool LDEBUG) {
    // system AB3C3_cF112_227_c_de_f

    if(XHOST.vflag_control.flag("WWW")) {
      WebANRL_AB3C3_cF112_227_c_de_f(web,LDEBUG); // PLUG WEB STUFF
#ifdef _ANRL_NOWEB_
      web << "no web";
      cout << web.str() << endl;
#else
    ...
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB3C3_cF112_227_c_de_f: FOUND" << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB3C3_cF112_227_c_de_f: label=" << label << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB3C3_cF112_227_c_de_f: nspecies=" << nspecies << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB3C3_cF112_227_c_de_f: natoms=" << natoms << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB3C3_cF112_227_c_de_f: spacegroup=" << spacegroup << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB3C3_cF112_227_c_de_f: nunderscores=" << nunderscores << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB3C3_cF112_227_c_de_f: nparameters=" <<  nparameters << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB3C3_cF112_227_c_de_f: Pearson_symbol=" << Pearson_symbol << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB3C3_cF112_227_c_de_f: params=" << params << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB3C3_cF112_227_c_de_f: Strukturbericht=" << Strukturbericht << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB3C3_cF112_227_c_de_f: prototype=" << prototype << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB3C3_cF112_227_c_de_f: dialect=" << dialect << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB3C3_cF112_227_c_de_f: vparameters.size()=" << vparameters.size() << endl;}
    ...
    uint i=0;
    double a=vparameters.at(i++);                  if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB3C3_cF112_227_c_de_f: a=" << a << endl;}

    double x3=vparameters.at(i++);                 if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB3C3_cF112_227_c_de_f: x3=" << x3 << endl;}
    double x4=vparameters.at(i++);                 if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB3C3_cF112_227_c_de_f: x4=" << x4 << endl;}
    ...
    atom.fpos(1)=-x3;atom.fpos(2)=-x3;atom.fpos(3)=((1.0/2.0)+3.0*x3);                     // atom B13
    atom.fpos_equation.clear();atom.fpos_equation.push_back("-x3");atom.fpos_equation.push_back("-x3");atom.fpos_equation.push_back("((1.0/2.0)+3.0*x3)");// atom B13 // symbolic math for atom positions
    str.comp_each_type.at(atom.type)+=1.0;                            // atom B13 // if we need partial occupation
    str.atoms.push_back(atom);                                        // atom B13

    atom.name="B"; atom.type=1;                                       // atom B14
    atom.fpos(1)=-x3;atom.fpos(2)=-x3;atom.fpos(3)=-x3;                     // atom B14
    atom.fpos_equation.clear();atom.fpos_equation.push_back("-x3");atom.fpos_equation.push_back("-x3");atom.fpos_equation.push_back("-x3");// atom B14 // symbolic math for atom positions
    ...
    atom.fpos(1)=(x4+(3.0/4.0));atom.fpos(2)=-x4;atom.fpos(3)=(x4+(3.0/4.0));                     // atom B23
    atom.fpos_equation.clear();atom.fpos_equation.push_back("(x4+(3.0/4.0))");atom.fpos_equation.push_back("-x4");atom.fpos_equation.push_back("(x4+(3.0/4.0))");// atom B23 // symbolic math for atom positions
    str.comp_each_type.at(atom.type)+=1.0;                            // atom B23 // if we need partial occupation
    str.atoms.push_back(atom);                                        // atom B23

    atom.name="C"; atom.type=2;                                       // atom B24
    atom.fpos(1)=-x4;atom.fpos(2)=(x4+(3.0/4.0));atom.fpos(3)=-x4;                     // atom B24
    atom.fpos_equation.clear();atom.fpos_equation.push_back("-x4");atom.fpos_equation.push_back("(x4+(3.0/4.0))");atom.fpos_equation.push_back("-x4");// atom B24 // symbolic math for atom positions
    str.comp_each_type.at(atom.type)+=1.0;                            // atom B24 // if we need partial occupation
    str.atoms.push_back(atom);                                        // atom B24

    atom.name="C"; atom.type=2;                                       // atom B25
    atom.fpos(1)=-x4;atom.fpos(2)=(x4+(3.0/4.0));atom.fpos(3)=(x4+(3.0/4.0));                     // atom B25
    atom.fpos_equation.clear();atom.fpos_equation.push_back("-x4");atom.fpos_equation.push_back("(x4+(3.0/4.0))");atom.fpos_equation.push_back("(x4+(3.0/4.0))");// atom B25 // symbolic math for atom positions
    str.comp_each_type.at(atom.type)+=1.0;                            // atom B25 // if we need partial occupation
    str.atoms.push_back(atom);                                        // atom B25

    atom.name="C"; atom.type=2;                                       // atom B26
    atom.fpos(1)=(x4+(3.0/4.0));atom.fpos(2)=-x4;atom.fpos(3)=-x4;                     // atom B26
    atom.fpos_equation.clear();atom.fpos_equation.push_back("(x4+(3.0/4.0))");atom.fpos_equation.push_back("-x4");atom.fpos_equation.push_back("-x4");// atom B26 // symbolic math for atom positions
    str.comp_each_type.at(atom.type)+=1.0;                            // atom B26 // if we need partial occupation
    str.atoms.push_back(atom);                                        // atom B26

    atom.name="C"; atom.type=2;                                       // atom B27
    atom.fpos(1)=-x4;atom.fpos(2)=-x4;atom.fpos(3)=(x4+(3.0/4.0));                     // atom B27
    atom.fpos_equation.clear();atom.fpos_equation.push_back("-x4");atom.fpos_equation.push_back("-x4");atom.fpos_equation.push_back("(x4+(3.0/4.0))");// atom B27 // symbolic math for atom positions
    ...
    atom.fpos(1)=(x4+(3.0/4.0));atom.fpos(2)=(x4+(3.0/4.0));atom.fpos(3)=-x4;                     // atom B28
    atom.fpos_equation.clear();atom.fpos_equation.push_back("(x4+(3.0/4.0))");atom.fpos_equation.push_back("(x4+(3.0/4.0))");atom.fpos_equation.push_back("-x4");// atom B28 // symbolic math for atom positions
...
namespace anrl {
  uint WebANRL_AB3C3_cF112_227_c_de_f(stringstream& web,bool LDEBUG) {
    ...
    if(LDEBUG) {cerr << "anrl:: WebANRL_AB3C3_cF112_227_c_de_f: web.str().size()=" << web.str().size() << endl;}
