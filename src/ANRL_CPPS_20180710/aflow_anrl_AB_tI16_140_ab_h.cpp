...
// FILE "ANRL/aflow_anrl_AB_tI16_140_ab_h.cpp"

#ifndef _AFLOW_ANRL_AB_tI16_140_ab_h_CPP // AFLOW_REMOVE_GREP
#define _AFLOW_ANRL_AB_tI16_140_ab_h_CPP // AFLOW_REMOVE_GREP
#include "../aflow.h" // AFLOW_REMOVE_GREP

namespace anrl {
  uint WebANRL_AB_tI16_140_ab_h(stringstream &web,bool LDEBUG);
}

namespace anrl {
  uint PrototypeANRL_AB_tI16_140_ab_h(stringstream &web,xstructure& str,string parameters,string proto_line,uint print_mode,bool LDEBUG) {
    // system AB_tI16_140_ab_h

    if(XHOST.vflag_control.flag("WWW")) {
      WebANRL_AB_tI16_140_ab_h(web,LDEBUG); // PLUG WEB STUFF
      #ifdef _ANRL_NOWEB_
      cout << "no web" << endl;
      #else
      cout << web.str() << endl;
    ...
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_tI16_140_ab_h: FOUND" << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_tI16_140_ab_h: label=" << label << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_tI16_140_ab_h: nspecies=" << nspecies << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_tI16_140_ab_h: natoms=" << natoms << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_tI16_140_ab_h: spacegroup=" << spacegroup << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_tI16_140_ab_h: nunderscores=" << nunderscores << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_tI16_140_ab_h: nparameters=" <<  nparameters << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_tI16_140_ab_h: Pearson_symbol=" << Pearson_symbol << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_tI16_140_ab_h: params=" << params << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_tI16_140_ab_h: Strukturbericht=" << Strukturbericht << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_tI16_140_ab_h: prototype=" << prototype << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_tI16_140_ab_h: dialect=" << dialect << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_tI16_140_ab_h: vparameters.size()=" << vparameters.size() << endl;}
    ...
    uint i=0;
    double a=vparameters.at(i++);                  if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_tI16_140_ab_h: a=" << a << endl;}
    double covera=vparameters.at(i++),c=covera*a;  if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_tI16_140_ab_h: c=" << c << " (c/a=" << covera << ")" << endl;}
    
    double x3=vparameters.at(i++);                 if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_tI16_140_ab_h: x3=" << x3 << endl;}
...
namespace anrl {
  uint WebANRL_AB_tI16_140_ab_h(stringstream& web,bool LDEBUG) {
    ...
    if(LDEBUG) {cerr << "anrl:: WebANRL_AB_tI16_140_ab_h: web.str().size()=" << web.str().size() << endl;}
