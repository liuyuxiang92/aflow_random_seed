...
// FILE "ANRL/aflow_anrl_A3B_hP8_176_h_c.cpp"

#ifndef _AFLOW_ANRL_A3B_hP8_176_h_c_CPP // AFLOW_REMOVE_GREP
#define _AFLOW_ANRL_A3B_hP8_176_h_c_CPP // AFLOW_REMOVE_GREP
#include "../aflow.h" // AFLOW_REMOVE_GREP

namespace anrl {
  uint WebANRL_A3B_hP8_176_h_c(stringstream &web,bool LDEBUG);
}

namespace anrl {
  uint PrototypeANRL_A3B_hP8_176_h_c(stringstream &web,xstructure& str,string parameters,string proto_line,uint print_mode,bool LDEBUG) {
    // system A3B_hP8_176_h_c

    if(XHOST.vflag_control.flag("WWW")) {
      WebANRL_A3B_hP8_176_h_c(web,LDEBUG); // PLUG WEB STUFF
#ifdef _ANRL_NOWEB_
      web << "no web";
      cout << web.str() << endl;
#else
    ...
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_A3B_hP8_176_h_c: FOUND" << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_A3B_hP8_176_h_c: label=" << label << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_A3B_hP8_176_h_c: nspecies=" << nspecies << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_A3B_hP8_176_h_c: natoms=" << natoms << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_A3B_hP8_176_h_c: spacegroup=" << spacegroup << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_A3B_hP8_176_h_c: nunderscores=" << nunderscores << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_A3B_hP8_176_h_c: nparameters=" <<  nparameters << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_A3B_hP8_176_h_c: Pearson_symbol=" << Pearson_symbol << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_A3B_hP8_176_h_c: params=" << params << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_A3B_hP8_176_h_c: Strukturbericht=" << Strukturbericht << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_A3B_hP8_176_h_c: prototype=" << prototype << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_A3B_hP8_176_h_c: dialect=" << dialect << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_A3B_hP8_176_h_c: vparameters.size()=" << vparameters.size() << endl;}
    ...
    uint i=0;
    double a=vparameters.at(i++);                  if(LDEBUG) { cerr << "anrl::PrototypeANRL_A3B_hP8_176_h_c: a=" << a << endl;}
    double covera=vparameters.at(i++),c=covera*a;  if(LDEBUG) { cerr << "anrl::PrototypeANRL_A3B_hP8_176_h_c: c=" << c << " (c/a=" << covera << ")" << endl;}

    double x2=vparameters.at(i++);                 if(LDEBUG) { cerr << "anrl::PrototypeANRL_A3B_hP8_176_h_c: x2=" << x2 << endl;}
    double y2=vparameters.at(i++);                 if(LDEBUG) { cerr << "anrl::PrototypeANRL_A3B_hP8_176_h_c: y2=" << y2 << endl;}
...
namespace anrl {
  uint WebANRL_A3B_hP8_176_h_c(stringstream& web,bool LDEBUG) {
    ...
    if(LDEBUG) {cerr << "anrl:: WebANRL_A3B_hP8_176_h_c: web.str().size()=" << web.str().size() << endl;}
