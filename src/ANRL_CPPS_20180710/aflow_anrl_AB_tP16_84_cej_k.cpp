...
// FILE "ANRL/aflow_anrl_AB_tP16_84_cej_k.cpp"

#ifndef _AFLOW_ANRL_AB_tP16_84_cej_k_CPP // AFLOW_REMOVE_GREP
#define _AFLOW_ANRL_AB_tP16_84_cej_k_CPP // AFLOW_REMOVE_GREP
#include "../aflow.h" // AFLOW_REMOVE_GREP

namespace anrl {
  uint WebANRL_AB_tP16_84_cej_k(stringstream &web,bool LDEBUG);
}

namespace anrl {
  uint PrototypeANRL_AB_tP16_84_cej_k(stringstream &web,xstructure& str,string parameters,string proto_line,uint print_mode,bool LDEBUG) {
    // system AB_tP16_84_cej_k

    if(XHOST.vflag_control.flag("WWW")) {
      WebANRL_AB_tP16_84_cej_k(web,LDEBUG); // PLUG WEB STUFF
#ifdef _ANRL_NOWEB_
      cout << "no web" << endl;
#else
      cout << web.str() << endl;
    ...
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_tP16_84_cej_k: FOUND" << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_tP16_84_cej_k: label=" << label << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_tP16_84_cej_k: nspecies=" << nspecies << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_tP16_84_cej_k: natoms=" << natoms << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_tP16_84_cej_k: spacegroup=" << spacegroup << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_tP16_84_cej_k: nunderscores=" << nunderscores << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_tP16_84_cej_k: nparameters=" <<  nparameters << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_tP16_84_cej_k: Pearson_symbol=" << Pearson_symbol << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_tP16_84_cej_k: params=" << params << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_tP16_84_cej_k: Strukturbericht=" << Strukturbericht << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_tP16_84_cej_k: prototype=" << prototype << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_tP16_84_cej_k: dialect=" << dialect << endl;}
    if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_tP16_84_cej_k: vparameters.size()=" << vparameters.size() << endl;}
    ...
    uint i=0;
    double a=vparameters.at(i++);                  if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_tP16_84_cej_k: a=" << a << endl;}
    double covera=vparameters.at(i++),c=covera*a;  if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_tP16_84_cej_k: c=" << c << " (c/a=" << covera << ")" << endl;}

    double x3=vparameters.at(i++);                 if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_tP16_84_cej_k: x3=" << x3 << endl;}
    double y3=vparameters.at(i++);                 if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_tP16_84_cej_k: y3=" << y3 << endl;}
    double x4=vparameters.at(i++);                 if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_tP16_84_cej_k: x4=" << x4 << endl;}
    double y4=vparameters.at(i++);                 if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_tP16_84_cej_k: y4=" << y4 << endl;}
    double z4=vparameters.at(i++);                 if(LDEBUG) { cerr << "anrl::PrototypeANRL_AB_tP16_84_cej_k: z4=" << z4 << endl;}
    ...
    atom.fpos(1)=-y3;atom.fpos(2)=x3;atom.fpos(3)=(1.0/2.0);                     // atom B7
    atom.fpos_equation.clear();atom.fpos_equation.push_back("-y3");atom.fpos_equation.push_back("x3");atom.fpos_equation.push_back("(1.0/2.0)");// atom B7 // symbolic math for atom positions
    str.comp_each_type.at(atom.type)+=1.0;                            // atom B7 // if we need partial occupation
    str.atoms.push_back(atom);                                        // atom B7

    atom.name="A"; atom.type=0;                                       // atom B8
    atom.fpos(1)=y3;atom.fpos(2)=-x3;atom.fpos(3)=(1.0/2.0);                     // atom B8
    atom.fpos_equation.clear();atom.fpos_equation.push_back("y3");atom.fpos_equation.push_back("-x3");atom.fpos_equation.push_back("(1.0/2.0)");// atom B8 // symbolic math for atom positions
...
namespace anrl {
  uint WebANRL_AB_tP16_84_cej_k(stringstream& web,bool LDEBUG) {
    ...
    if(LDEBUG) {cerr << "anrl:: WebANRL_AB_tP16_84_cej_k: web.str().size()=" << web.str().size() << endl;}
