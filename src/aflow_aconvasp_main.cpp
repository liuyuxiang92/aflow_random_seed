...
uint PflowARGs(vector<string> &argv,vector<string> &cmds,aurostd::xoption &vpflow) {  // vpflow is really XHOST.vflag_pflow
  ...
  if(vpflow.flag("COMPARE_MATERIAL") || vpflow.flag("COMPARE_STRUCTURE")) {
    ...
    //DX20190424 [OBSOLETE]if(vpflow.flag("COMPARE_MATERIAL") || vpflow.flag("COMPARE_STRUCTURE")) { //[CO200106 - close bracket for indenting]}
  ...
  if(vpflow.flag("PROTO_AFLOW")) {
    ...
    vpflow.flag("PROTO_AFLOW::ABCCAR",aurostd::args2flag(argv,cmds,"--abccar"));  //DX190131                                                                     //DX20190123 - added CIF
    if(vpflow.flag("PROTO_AFLOW::ABCCAR")) vlist+="--abccar ";  //DX190131                                              // recursion is GNU's pleasure (SC2016)  //DX20190123 - added CIF
    ...
    vpflow.flag("PROTO_AFLOW::CIF",aurostd::args2flag(argv,cmds,"--cif"));  //DX190131                                                                           //DX20190123 - added CIF
    if(vpflow.flag("PROTO_AFLOW::CIF")) vlist+="--cif ";  //DX190131                                                    // recursion is GNU's pleasure (SC2016)  //DX20190123 - added CIF
...
namespace pflow {
  int main(vector<string> &argv,vector<string> &cmds) {
    ...
    if(argv.size()>=2 && !_PROGRAMRUN) {
      ...
      //[CO190208 - OBSOLETE]if(vpflow.flag("HNF")) {pflow::HNF(vpflow,cin,cout); _PROGRAMRUN=true;}  //CO20181226
      //[CO190208 - OBSOLETE]if(vpflow.flag("HNFCELL")) {pflow::HNFCELL(vpflow,cin,cout); _PROGRAMRUN=true;} //CO20181226
...
namespace pflow {
  bool SYMMETRY_GROUPS(_aflags &aflags,istream& input, aurostd::xoption& vpflow, ostream& oss){
    ...
    string directory=aurostd::getPWD(); //[CO191112 - OBSOLETE]aurostd::execute2string("pwd"); // DX20180221 - use pwd
    if(XHOST.vflag_control.flag("DIRECTORY")) {
...
namespace pflow {
  ...
  //[CO190208 - OBSOLETE]bool HNF(aurostd::xoption& vpflow,istream& input,ostream& oss) {  //CO20181226
...
namespace pflow {
  string LATTICE_LATTICE_TYPE(istream& input) {
    ...
    // sss << " Real space Pearson symbol              = " << a.pearson_symbol << endl;
    sss << str_sp.bravais_lattice_lattice_type << "," << str_sp.bravais_lattice_lattice_variation_type << endl; //wahyu mod /DX20170824 - a to str_sp
...
namespace pflow {
  ...
  { //CO200106 - patching for auto-indenting
    ...
    //[OBSOLETE CO20180703]  { //CO200106 - patching for auto-indenting
