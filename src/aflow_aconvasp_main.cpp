    ...
    if(vpflow.flag("CHULL::OUTPUT")){
      ...
      for(uint i=0;i<out_forms.size();i++){
        ...
        if(out_form.at(0)=='A'){
          ...
          //[WSCHMITT190620 - stability criterion error otherwise for Pd1]vpflow.flag("CHULL::SKIP_STRUCTURE_COMPARISON",TRUE); //the app cannot handle more than one g-state in the visualization
