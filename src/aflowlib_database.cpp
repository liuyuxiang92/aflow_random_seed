...
namespace aflowlib {
  ...
  void AflowDB::openTmpFile(int open_flags, bool copy_original) {
    ...
    if (symlink(lock_file.c_str(), lock_link.c_str())) {
      ...
      if (errno == EEXIST) {
        ...
        throw aurostd::xerror(_AFLOW_FILE_NAME_, __func__, message, _RUNTIME_BUSY_);
      } else {
    ...
    if (aurostd::FileExist(tmp_file)) {
      ...
      bool del_tmp = false;
      if (LDEBUG) {
        std::cerr << __func__ << " Found temporary database file with time stamp " << tm_tmp
  ...
  bool AflowDB::closeTmpFile(bool force_copy, bool keep, bool nocopy) {
    ...
    if (sql_code != SQLITE_OK) {
      ...
      throw aurostd::xerror(_AFLOW_FILE_NAME_, __func__, message_error, _FILE_ERROR_);
    }
    is_tmp = false;
    ...
    if (found_error || nocopy) {
      ...
      if (!aurostd::FileEmpty(database_file)) {
        ...
        if (LDEBUG) std::cerr << __func__ << "No old datbase found." << std::endl;
        ncols_old = 0;
        nentries_old = 0;
      }

      if (LDEBUG) {
        std::cerr << __func__ << "Number of entries: " << nentries_tmp << " (current database: " << nentries_old << ")." << std::endl;
        std::cerr << __func__ << "Number of properties: " << ncols_tmp << " (current database: " << ncols_old << ")." << std::endl;
      }
...
namespace aflowlib {
    ...
    for (uint i = 0; i < npatch_input; i++) {
      ...
      if (!aurostd::EFileExist(filename)) {
        ...
        if (aurostd::EFileExist(data_path + "/" + filename)) {
          ...
          pflow::logger(_AFLOW_FILE_NAME_, __func__, message, *p_FileMESSAGE, *p_oss, _LOGGER_WARNING_);
...
#endif
    message << __func__ << "Finished rebuild.";
    pflow::logger(_AFLOW_FILE_NAME_, __func__, message, *p_FileMESSAGE, *p_oss);
  }
  ...
  void AflowDB::populateTable(const string& table, const vector<string>& columns, const vector<string>& types, const vector<vector<string> >& values) {
    ...
    transaction(false);
    if (LDEBUG) std::cerr << __func__ <<  " Finished building table " << table << std::endl;
  }
