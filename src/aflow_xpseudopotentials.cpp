...
xPOTCAR xPOTCAR_Finder(vector<string>& species_pp_AUID,vector<string>& species_pp_AUID_collisions,const string& TITEL,const string& LEXCH,const double& EATOM,const double& RMAX,bool LVERBOSE) {
  ...
  for(uint ipp=0;ipp<vxpseudopotential.size();ipp++) {
	...
	//if(LVERBOSE)
	cerr << XHOST.sPID << "xPOTCAR::xPOTCAR_Finder: COLLISION: POTCAR " << vxpseudopotential.at(ipp).filename << " " << xPOT.filename << endl;
	species_pp_AUID_collisions.push_back(vxpseudopotential.at(ipp).AUID);
      }
    }
    if(test && !found) {
      found=TRUE;
      if(LVERBOSE) cerr << XHOST.sPID << "xPOTCAR::xPOTCAR_Finder: FOUND: POTCAR=" << vxpseudopotential.at(ipp).filename << endl;
  ...
  if(!found) {
    if(vxpseudopotential.size()) {
      // if(LVERBOSE)
      cerr << XHOST.sPID << "xPOTCAR::xPOTCAR_Finder: NOT FOUND: TITEL=" << TITEL << " LEXCH=" <<  LEXCH << " EATOM=" <<  EATOM << " RMAX =" <<  RMAX << endl;
      //    cout << XHOST.sPID << "xPOTCAR::xPOTCAR_Finder: NOT FOUND: TITEL=" << TITEL << " LEXCH=" <<  LEXCH << " EATOM=" <<  EATOM << " RMAX =" <<  RMAX << endl;
      // exit(0);
    }
...
xPOTCAR xPOTCAR_Finder(const string& AUID,bool LVERBOSE) {
  ...
  if(!found) {
    if(vxpseudopotential.size()) {
      // if(LVERBOSE)
      cerr << XHOST.sPID << "xPOTCAR::xPOTCAR_Finder: NOT FOUND: AUID=" << AUID << endl;
      //   cout << XHOST.sPID << "xPOTCAR::xPOTCAR_Finder: NOT FOUND: AUID=" << AUID << endl;
      //    exit(0);
    }
  }
...
bool xPOTCAR_PURE_Printer(xPOTCAR& xPOT,ostream& oss,bool LVERBOSE) {
  if(XHOST.PSEUDOPOTENTIAL_GENERATOR && xPOT.species.size()==1) {  //SC20200326
    ...
    if(found)  cerr << XHOST.sPID << "xPOTCAR_PURE_Printer:     FOUND AUID=" << xPOT.AUID << endl;
    if(!found) cerr << XHOST.sPID << "xPOTCAR_PURE_Printer: NOT_FOUND AUID=" << xPOT.AUID << endl;
...
bool xPOTCAR_EnthalpyReference_AUID(string AUID,string METAGGA,string& groundstate_structure,double& groundstate_energy,double& volume_atom,double& spin_atom) {
  bool LDEBUG=(FALSE || XHOST.DEBUG);
  bool VERBOSE=0;//TRUE;
  if(LDEBUG) cerr << XHOST.sPID << "xPOTCAR_EnthalpyReference_AUID: [BEGIN]" << endl;
  bool found=FALSE;
  ...
  if(AUID=="6bf17162620b7ce3" && nKIN) {found=TRUE;groundstate_structure="diatom";groundstate_energy=-1.787206;volume_atom=500;spin_atom=0.0;} // Cl:PAW_PBE:17Jan2003  RICO
  if(AUID=="9d680135d96b4029" && SCAN) {found=TRUE;groundstate_structure="diatom";groundstate_energy=-7.906078;volume_atom=500;spin_atom=0.0;} // Cl:PAW_PBE_KIN:SCAN:06Sep2000 RICO
  ...
  // N
  if(AUID=="4ce4637079c90d89" && nKIN) {found=TRUE;groundstate_structure="diatom";groundstate_energy=-8.31918;volume_atom=500;spin_atom=0.0;} // N:PAW_PBE_KIN:08Apr2002
  if(AUID=="3cd4003f27559a49" && nKIN) {found=TRUE;groundstate_structure="diatom";groundstate_energy=-8.3187;volume_atom=500;spin_atom=0.0;} // N:PAW_PBE:08Apr2002
  ...
  // O
  if(AUID=="5c530b0eaed90a00" && nKIN) {found=TRUE;groundstate_structure="diatom";groundstate_energy=-4.93107;volume_atom=500;spin_atom=1.0;} // O:PAW_PBE_KIN:08Apr2002
  if(AUID=="85ded82734544fa9" && nKIN) {found=TRUE;groundstate_structure="diatom";groundstate_energy=-4.93114;volume_atom=500;spin_atom=1.0;} // O:PAW_PBE:08Apr2002
 ...
 if(AUID=="1e59564cdfe6dd0a" && nKIN) {found=TRUE;groundstate_structure="A3";groundstate_energy=-12.34746;volume_atom=14.96078;spin_atom=0.0;} // Re:PAW_GGA:05Jan2001
  ...
  if(LDEBUG) cerr << XHOST.sPID << "xPOTCAR_EnthalpyReference_AUID: [END]" << endl;
  return found;
};
