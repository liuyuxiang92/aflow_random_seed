...
#ifndef _AFLOW_APENNSY_VASPIN_CPP_
#define _AFLOW_APENNSY_VASPIN_CPP_
#include "aflow.h"

// ***************************************************************************
#define APENNSY_BUFFER_MAXLEN 1024
#define APENNSY_AAmax 20
#define APENNSY_BBmax 20
#define APENNSY_MAXLINESOSTREAM 64

// **************************************************************************
// LatexSideMarginThesisON - routine for printing latex
// **************************************************************************
...
// **************************************************************************
// LatexSideMarginThesisOFF - routine for printing latex
// **************************************************************************
namespace apennsy_std {
...
// **************************************************************************
// Rational - routine for printing latex
// **************************************************************************
namespace apennsy_std {
  void Rational(ostream &os,double x) {
    if(x<0) os << "-";
    x=abs(x);
    if(aurostd::isequal(x,0.50000000000000,CEPSILON)) {os << "1/2"; return;};
    if(aurostd::isequal(x,0.08333333333333,CEPSILON)) {os << "1/12"; return;};
    if(aurostd::isequal(x,0.41666666666667,CEPSILON)) {os << "5/12"; return;};
    if(aurostd::isequal(x,0.58333333333333,CEPSILON)) {os << "7/12"; return;};
    if(aurostd::isequal(x,0.91666666666667,CEPSILON)) {os << "11/12"; return;};
    if(aurostd::isequal(x,1.50000000000000,CEPSILON)) {os << "3/2"; return;};
    if(aurostd::isequal(x,2.50000000000000,CEPSILON)) {os << "5/2"; return;};
    if(aurostd::isequal(x,0.33333333333333,CEPSILON)) {os << "1/3"; return;};
    if(aurostd::isequal(x,0.66666666666666,CEPSILON)) {os << "2/3"; return;};
    if(aurostd::isequal(x,1.33333333333333,CEPSILON)) {os << "4/3"; return;};
    if(aurostd::isequal(x,1.66666666666666,CEPSILON)) {os << "5/3"; return;};
    if(aurostd::isequal(x,2.33333333333333,CEPSILON)) {os << "7/3"; return;};
    if(aurostd::isequal(x,2.66666666666666,CEPSILON)) {os << "8/3"; return;};
    if(aurostd::isequal(x,0.12500000000000,CEPSILON)) {os << "1/8"; return;};
    if(aurostd::isequal(x,0.25000000000000,CEPSILON)) {os << "1/4"; return;};
    if(aurostd::isequal(x,0.37500000000000,CEPSILON)) {os << "3/8"; return;};
    if(aurostd::isequal(x,0.62500000000000,CEPSILON)) {os << "5/8"; return;};
    if(aurostd::isequal(x,0.75000000000000,CEPSILON)) {os << "3/4"; return;};
    if(aurostd::isequal(x,0.87500000000000,CEPSILON)) {os << "7/8"; return;};
    if(aurostd::isequal(x,0.11111111111111,CEPSILON)) {os << "1/9"; return;};
    if(aurostd::isequal(x,0.22222222222222,CEPSILON)) {os << "2/9"; return;};
    if(aurostd::isequal(x,0.44444444444444,CEPSILON)) {os << "4/9"; return;};
    if(aurostd::isequal(x,0.55555555555555,CEPSILON)) {os << "5/9"; return;};
    if(aurostd::isequal(x,0.77777777777777,CEPSILON)) {os << "7/9"; return;};
    if(aurostd::isequal(x,0.88888888888888,CEPSILON)) {os << "8/9"; return;};
    if(aurostd::isequal(x,1.73205080756888,CEPSILON)) {os << "\\sqrt{3}"; return;};
    if(aurostd::isequal(x,2.59807621135332,CEPSILON)) {os << "3\\sqrt{3}/2"; return;};
    if(aurostd::isequal(x,1.63299316185545,CEPSILON)) {os << "\\sqrt{8/3}"; return;};
    if(aurostd::isequal(x,3.26598632371090,CEPSILON)) {os << "2\\sqrt{8/3}"; return;};
    if(aurostd::isequal(x,0.86602540378444,CEPSILON)) {os << "\\sqrt{3}/2"; return;};
    if(aurostd::isequal(x,0.16666666666667,CEPSILON)) {os << "1/6"; return;};
    if(aurostd::isequal(x,0.83333333333333,CEPSILON)) {os << "5/6"; return;};
    if(aurostd::isequal(x,0.10000000000000,CEPSILON)) {os << "1/10"; return;};
    if(aurostd::isequal(x,0.20000000000000,CEPSILON)) {os << "1/5"; return;};
    if(aurostd::isequal(x,0.30000000000000,CEPSILON)) {os << "3/10"; return;};
    if(aurostd::isequal(x,0.40000000000000,CEPSILON)) {os << "4/10"; return;};
    if(aurostd::isequal(x,0.60000000000000,CEPSILON)) {os << "3/5"; return;};
    if(aurostd::isequal(x,0.70000000000000,CEPSILON)) {os << "7/10"; return;};
    if(aurostd::isequal(x,0.80000000000000,CEPSILON)) {os << "4/5"; return;};
    if(aurostd::isequal(x,0.90000000000000,CEPSILON)) {os << "9/10"; return;};
    if(aurostd::isequal(x,0.05555555555556,CEPSILON)) {os << "1/18"; return;};
    if(aurostd::isequal(x,0.27777777777778,CEPSILON)) {os << "5/18"; return;};
    if(aurostd::isequal(x,0.38888888888889,CEPSILON)) {os << "7/18"; return;};
    if(aurostd::isequal(x,0.61111111111111,CEPSILON)) {os << "11/18"; return;};
    if(aurostd::isequal(x,0.72222222222222,CEPSILON)) {os << "13/18"; return;};
    if(aurostd::isequal(x,0.94444444444444,CEPSILON)) {os << "17/18"; return;};
    if(aurostd::isequal(x,1.41421356237310,CEPSILON)) {os << "\\sqrt{2}"; return;};
    if(aurostd::isequal(x,2.23606797749979,CEPSILON)) {os << "\\sqrt{5}"; return;};
    if(aurostd::isequal(x,2.64575131106459,CEPSILON)) {os << "\\sqrt{7}"; return;};
    if(aurostd::isequal(x,3.16227766016838,CEPSILON)) {os << "\\sqrt{10}"; return;};
    if(aurostd::isequal(x,3.31662479035540,CEPSILON)) {os << "\\sqrt{11}"; return;};
    if(aurostd::isequal(x,3.46410161513775,CEPSILON)) {os << "\\sqrt{12}"; return;};
    if(aurostd::isequal(x,3.60555127546399,CEPSILON)) {os << "\\sqrt{13}"; return;};
    if(aurostd::isequal(x,3.74165738677394,CEPSILON)) {os << "\\sqrt{14}"; return;};
    if(aurostd::isequal(x,3.87298334620742,CEPSILON)) {os << "\\sqrt{15}"; return;};
    os << x ; return;
  }
}

// **************************************************************************
// read_VASPIN - routine for printing latex
// **************************************************************************
namespace apennsy_std {
  void read_VASPIN(ostringstream* l,
    ...
    xmatrix<double> Apos(APENNSY_AAmax,3),Bpos(APENNSY_BBmax,3);
    int j=1;
    l[j++] << "& " << _strukturbericht << "\t";  // STRUKTURBERICHT
    l[j++] << "& " << _proto << "\t";  // PROTOTYPE
    l[j++] << "& " << _labels << "\t";  // LABEL
    { // OPEN VASPIN FILE
      for(int i=0;i<APENNSY_BUFFER_MAXLEN;i++) aus0<<c;
      aus0.seekp(0,ios_base::beg);
      aus0 << number;
      ...
      if(!File.good()) {
        throw aurostd::xerror(__AFLOW_FILE__,__AFLOW_FUNC__,"file not found: "+FileName,_FILE_CORRUPT_); //CO20200624
      }
    }
    { // GET AA and BB NUMBERS
      for(int i=0;i<APENNSY_BUFFER_MAXLEN;i++) aus1<<c;
    ...
    { // GET SPACEGROUP
      ...
      //   l[j] << "&{" << APENNSY_LATEX_FONT << " " << spacegroup << "}\t";j++;
    }
    { // GET PEARSON_NUMBER
      aus0.seekp(0,ios_base::beg);for(int i=0;i<APENNSY_BUFFER_MAXLEN;i++) aus0<<c;aus0.seekp(0,ios_base::beg);  // RESET
...
// **************************************************************************
// MakeVaspIn - routine for printing latex
// **************************************************************************
namespace apennsy_std {
  void MakeVaspIn(bool VERB) {
    ...
    if(_FCC) {
      { // FCC1a TABLE
        ...
        ostringstream lines[APENNSY_MAXLINESOSTREAM];
        j=1;
...
// **************************************************************************
// ZungerConversionChart - routine for printing latex
// **************************************************************************
namespace apennsy_std {
