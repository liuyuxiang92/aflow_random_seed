...
namespace compare {
  ...
  string compareInputStructures(const aurostd::xoption& vpflow, ostream& logstream){ //DX20190425 - changed name, more general
    ...
    if(structures_source=="structure_list") {
      ...
      pflow::logger(_AFLOW_FILE_NAME_, __AFLOW_FUNC__, message, FileMESSAGE, logstream, _LOGGER_MESSAGE_);
    }
    //DX20190424 END
      ...
      if(!aurostd::FileExist(directory)) {
        ...
        pflow::logger(_AFLOW_FILE_NAME_, __AFLOW_FUNC__, message, FileMESSAGE, logstream, _LOGGER_ERROR_);
        return oss.str();
      }
    ...
    if(file_list.size()==2 && prototypes_final.size() > 0){
      ...
      if(prototypes_final[0].mapping_info_duplicate.size()==1){
        ...
        if(XHOST.QUIET){
          ...
          pflow::logger(_AFLOW_FILE_NAME_, __AFLOW_FUNC__, message, FileMESSAGE, logstream, _LOGGER_COMPLETE_);
        }
        if(print){
  ...
  string isopointalPrototypes(istream& input, const aurostd::xoption& vpflow){
    ...
    if(vpflow.flag("ISOPOINTAL_PROTOTYPES::CATALOG")) {
      ...
      if(catalog!="htqc" && catalog!="anrl" && catalog!="all"){
        throw aurostd::xerror(_AFLOW_FILE_NAME_, __AFLOW_FUNC__, "Catalog/library can only be htqc, anrl, or all.",_INPUT_ILLEGAL_);
      }
    }
...
vector<StructurePrototype> XtalFinderCalculator::compare2prototypes(
  ...
  if(vpflow.flag("COMPARE2PROTOTYPES::CATALOG")) {
    ...
    pflow::logger(_AFLOW_FILE_NAME_, __AFLOW_FUNC__, message, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_);
  }
  ...
  if(comparison_options.flag("COMPARISON_OPTIONS::CALCULATE_UNIQUE_PERMUTATIONS")){
    ...
    pflow::logger(_AFLOW_FILE_NAME_, __AFLOW_FUNC__, message, *p_FileMESSAGE, *p_oss, _LOGGER_COMPLETE_);
  }
...
vector<StructurePrototype> XtalFinderCalculator::compare2database(
  ...
  if(property_list.size() && relaxation_step != _COMPARE_DATABASE_GEOMETRY_MOST_RELAXED_){
    ...
    else { throw aurostd::xerror(_AFLOW_FILE_NAME_, __AFLOW_FUNC__, "Unexpected relaxation step input: "+aurostd::utype2string<uint>(relaxation_step)+".", _INPUT_ERROR_); }
  ...
  pflow::logger(_AFLOW_FILE_NAME_, __AFLOW_FUNC__, message, *p_FileMESSAGE, *p_oss, _LOGGER_MESSAGE_); //DX20190403
...
namespace compare {
  ...
  string compareDatabaseEntries(const aurostd::xoption& vpflow, ofstream& FileMESSAGE, ostream& logstream){ //DX20191125 - added ofstream and ostream
    ...
    pflow::logger(_AFLOW_FILE_NAME_, __AFLOW_FUNC__, message, FileMESSAGE, logstream, _LOGGER_MESSAGE_); //DX20190403
